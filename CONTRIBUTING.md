...
FactorZen 当前聚焦低频因子评估与报告。优先接受能提升**研究可信度、可复现性、测试覆盖与文档清晰度**的改动。
...
所有命令通过 `pixi run` 执行。`.env` 只放本地凭据，不提交到仓库。

如本机已安装 `pre-commit`，可启用本地钩子：
...
- 提交信息遵循 Conventional Commits，例如 `fix:`、`feat:`、`docs:`、`test:`、`chore:`。
- bugfix 与核心行为变更应先有能复现问题的测试，再实现修复。
- 涉及收益、价格、成交约束或样本切分的改动，必须说明是否可能引入未来函数，并补充相应回归测试。
- 不提交本地行情数据、运行产物、日志、notebook checkpoint、`.env` 或任何 token。

## 编码风格

- Python 代码由 `ruff` 与 `ruff-format` 统一处理。
- 中文文档与注释使用 UTF-8 保存。
