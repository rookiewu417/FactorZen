...
| **M0 微观结构** | 交易约束（停牌/涨跌停/ST/次新/T+1）+ universe 快照 + 性能基准 | `core/`（universe/benchmark）`daily/evaluation/` | （内嵌回测） |
| **M1 因子挖掘** | 算子库 + 表达式 AST↔字符串编译 + 随机/遗传搜索 + 贪心去相关 | `discovery/` | `fz mine search` |
| **M2 防过拟合** | block bootstrap IC CI + Deflated Sharpe + PBO/CSCV + holdout 永久隔离 | `validation/` | `fz validate overfit` |
| **M3 风险模型** | Barra 风格（8）+ 行业因子暴露 + Newey-West 协方差 + 特质风险收缩 + MCR 分解 | `risk/` | `fz risk build` |
| **M4 组合优化+归因** | cvxpy 因子形式 mean-variance QP + box/budget/行业风格中性/换手约束；Brinson + 风险因子归因 | `portfolio/` `attribution/` | `fz portfolio build` |
| **M5 单 Agent 挖掘** | LLM 闭环（假设→生成→护栏→critic→反思），零依赖自建 loop，Negative RAG | `agents/` | `fz mine agent` |
| **M6 多 Agent 团队** | 5 角色（Hypothesis/Coder/Critic/Librarian/Evaluator）+ 跨轮否决 + 跨 session 长期记忆 | `agents/roles/` `team_orchestrator.py` `experiment_index.py` | `fz mine team` |
| **M7 模拟交易** | 多周期权重回测（对齐行情/扣换手成本/净值/夏普/最大回撤） | `sim/` | `fz sim run` |
| **M7 成果展示** | 组合绩效 HTML Dashboard（指标卡+净值曲线+月度热图+归因+风险摘要） | `reports/portfolio_report.py` | `fz report portfolio` |
...
`.env` 不入库。真实数据拉取需要配置 `TUSHARE_TOKEN`；LLM 挖掘功能（M5/M6）需要配置 `FACTORZEN_LLM_*`，缺少时自动跳过。
...
# 2. 防过拟合验收（holdout 段 + Deflated Sharpe + bootstrap CI）
pixi run fz validate overfit <factor> --start 20200101 --end 20241231

# 3. 建风险模型（Barra 因子暴露 + Newey-West 协方差）
pixi run fz risk build --start 20200101 --end 20241231

# 4. 组合优化建仓（α + 风险模型 → 目标权重 + 归因）
pixi run fz portfolio build --start 20200101 --end 20241231 \
  --alpha-file alpha.parquet --industry-neutral

# 5. 模拟交易（组合权重 → 多周期净值/绩效）
pixi run fz sim run --portfolio-dir workspace/portfolios --start 20200101 --end 20241231

# 6. 成果展示页（指标卡 + 净值曲线 + 归因 → HTML Dashboard）
pixi run fz report portfolio \
  --sim-dir workspace/sim/<id> --portfolio-dir workspace/portfolios/<id>
```
...
├── risk_models/{run_id}/          Barra 风险模型（因子暴露 + 协方差矩阵）
├── portfolios/{run_id}/           组合权重 + 归因结果
├── sim/{run_id}/                  模拟净值 + 绩效摘要
└── reports/{run_id}/              组合绩效 HTML Dashboard
```
...
├── core/                 日历、universe、benchmark、存储、实验元数据
├── builtin_factors/      框架自带因子（daily/weekly/monthly/intraday/qlib），随包分发
├── daily/                低频主线：数据、因子、预处理、评估与优化
├── discovery/            M1 因子挖掘：算子库 + 表达式 AST + 随机/遗传搜索
├── validation/           M2 防过拟合：bootstrap IC CI + Deflated Sharpe + PBO/CSCV
├── risk/                 M3 Barra 风险模型：因子暴露 + Newey-West 协方差
├── portfolio/            M4 组合优化：cvxpy mean-variance QP + 约束体系
├── attribution/          M4 绩效归因：Brinson + 风险因子归因
├── agents/               M5/M6 LLM 挖掘：单 Agent 闭环 + 多 Agent 团队
├── sim/                  M7 模拟交易：多周期净值回测
...
└── configs/              实验 YAML 配置
tests/                    pytest 测试（1109 个）
docs/                     架构、运行手册、因子编写、M1-M7 specs/plans
```
...
- **工程**：pydantic / pyyaml / apscheduler
- **质量**：1109 pytest 测试 / ruff / mypy；CLI 入口 `pixi run fz`
...
| [发布记录 v0.3.0](docs/release-notes/v0.3.0.md) | M0-M7 完整升级变更日志 |
| [端到端教程](docs/end-to-end-tutorial.md) | 手把手六步走：从拉数据到组合 Dashboard |
| [M1 挖掘引擎 spec](docs/superpowers/specs/2026-06-29-m1-factor-mining-design.md) | 算子库 + AST + 搜索引擎设计 |
| [M2 防过拟合 spec](docs/superpowers/specs/2026-06-30-m2-overfit-guards-design.md) | bootstrap IC CI + Deflated Sharpe + PBO 设计 |
| [M3 风险模型 spec](docs/superpowers/specs/2026-06-30-m3-risk-model-finalization-design.md) | Barra 多因子风险模型设计 |
| [M4 组合优化 spec](docs/superpowers/specs/2026-06-30-m4-portfolio-optimization-design.md) | mean-variance QP + 归因设计 |
| [M5 单 Agent spec](docs/superpowers/specs/2026-06-30-m5-llm-agent-mining-design.md) | LLM 单 Agent 挖掘闭环设计 |
| [M6 多 Agent spec](docs/superpowers/specs/2026-06-30-m6-multi-agent-mining-design.md) | 多 Agent 团队 + 长期记忆设计 |
