...
> **FactorZen** 是一个面向 A 股单因子的可信研究框架，强调严谨、克制和可复现。当前核心主线覆盖因子计算、预处理、IC/回测评估、walk-forward OOS、数据质量报告、实验 manifest 与 Tear Sheet 报告生成。`daily/combination/` 已提供实验性多因子合成工具，用于研究对比，不作为当前生产组合优化模块。
