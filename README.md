...
| 挖掘 | 表达式因子挖掘：算子库 + AST↔字符串编译、随机/遗传搜索、贪心去相关，`fz mine` + LLM Agent 闭环（`fz mine agent`）+ 多 Agent 团队（`fz mine team`） |
...
| 组合 | 带约束凸优化(cvxpy mean-variance + 行业/风格中性 + 换手)、Brinson + 风险因子归因，`fz portfolio build` |
| 模拟交易 | 多周期组合权重回测、净值/换手/成本/夏普计算，`fz sim run` |
| 成果展示页 | 组合绩效 HTML dashboard（指标卡 + 归因 + 风险摘要），`fz report portfolio` |
