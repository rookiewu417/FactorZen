...
| **微观结构与交易约束** | 交易约束（停牌/涨跌停/ST/次新/T+1）+ universe 快照 + 性能基准 | `core/`（universe/benchmark）`daily/evaluation/` | （内嵌回测） |
...
- **质量**：1111 pytest 测试 / ruff / mypy；CLI 入口 `pixi run fz`
