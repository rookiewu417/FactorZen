...
from automation.state import run_record
...
def _with_retry(func, *args, max_retries: int = SCHEDULER_MAX_RETRIES,
                base_seconds: int = SCHEDULER_RETRY_BASE_SECONDS, **kwargs) -> None:
    """带指数退避重试的函数调用包装器。
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                ...
                logger.error(
                    f"[retry] {func.__name__} 达到重试上限 ({max_retries + 1} 次): {exc}"
                )
    if last_exc is not None:
        raise last_exc


def _run_daily_pipeline(date: str, factor_list: list[str], benchmark: str) -> None:
    """执行完整的日终流水线。
    ...
    if not is_trade_date(date):
        logger.info(f"[pipeline] {date} 非交易日，跳过流水线")
        return
