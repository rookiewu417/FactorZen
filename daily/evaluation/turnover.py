...
def compute_turnover(
    ...
    daily_turnover = (
        ...
        .with_columns(
            (pl.col("group") != pl.col("prev_group")).cast(pl.Float64).alias("changed")
        )
        .group_by("trade_date")
    ...
    migration = migration.join(total_per_from, on="prev_group").with_columns(
        (pl.col("count") / pl.col("total")).alias("prob")
    ).select(["prev_group", "group", "prob"]).sort(["prev_group", "group"])
