...
class Beta60D(LFTFactor):
    ...
    def compute(self, ctx: FactorDataContext) -> pl.DataFrame:
        ...
        df = df.sort(["ts_code", "trade_date"]).with_columns([
            pl.col("ret").rolling_mean(60, min_samples=30).over("ts_code").alias("_ret_mean"),
            pl.col("market_ret").rolling_mean(60, min_samples=30).over("ts_code").alias("_mkt_mean"),
        ]).with_columns([
            ((pl.col("ret") - pl.col("_ret_mean")) * (pl.col("market_ret") - pl.col("_mkt_mean")))
            .rolling_mean(60, min_samples=30).over("ts_code").alias("_cov"),
            ((pl.col("market_ret") - pl.col("_mkt_mean")) ** 2)
            .rolling_mean(60, min_samples=30).over("ts_code").alias("_var_m"),
        ]).with_columns(
            (pl.col("_cov") / (pl.col("_var_m") + 1e-12)).alias("factor_value")
        )
