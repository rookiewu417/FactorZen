...
三条原则贯穿全平台。设计冲突时以它们为裁决依据——包括「为了性能牺牲一点点 PIT」这类看起来划算的交易，也一律不做。
...
> ⚠️ **已知例外：美股 universe 不是 PIT。**
> `markets/us/sp500_snapshot.py` 用的是约 2024 年的静态成分快照（约 490 支），不是历史 point-in-time 成分。代码 docstring 自认「用它回看历史窗口会引入幸存者偏差」。用美股市场做历史回看时，这个偏差需要你自己承担和折算。A 股侧的 PIT 是严格的。
