...
> **只收有增量的因子。单因子指标仅作排序；lift 增量检验才是入库裁决。**

默认语境是 A 股（`--market ashare`）。多市场取值与能力边界见[多市场](multi-market.md)。
...
单因子指标衡量的是「这个因子相对于零有没有信号」。研究真正要回答的是「这个因子相对于**我已经拥有的东西**有没有信号」。因子库积累到一定规模后，这两个问题会给出完全相反的答案；库越大，分歧越严重。

FactorZen 把后者作为**最终裁决**：
...
代价是准入变贵——每评一个候选都要重跑「基线 vs 基线+候选」。平台为此做了基线复用、库池子进程隔离等优化，见[性能与资源](../guides/performance.md)。
...
关键在于它是**配对**统计——同一折、同一 universe、同一窗口下的两个组合直接相减，共同的市场噪声被抵消，剩下的才是候选因子的净贡献。
...
**后半段决定是 `active` 还是 `probation`。** 把评估窗口切两半，若后半段的增量为正，说明贡献不是集中在样本前期的一次性效应，直接给 `active`；否则先收进 `probation`，等真实时间累积更多证据再定。
...
`correlated` 这一态体现了平台的去相关策略：**高相关因子仍然收录并打标，而不是直接丢弃**。相关性是相对当前库的快照结论——库演化后，今天被判定冗余的因子可能明天就有位置。丢弃会丢失信息，打标则保留复审可能。
...
`--max-backfill-days` 默认只允许极少量回灌，本质上是拒绝用历史数据把观察期「补」出来——如果允许无限回灌，向前确认就退化成又一次样本内检验，失去全部意义。
