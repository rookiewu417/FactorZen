...
因子研究的核心风险不是「找不到信号」，而是「找到了不存在的信号」。搜索空间越大、迭代越多，这个风险越高。本文说明平台用哪些统计方法对抗它，以及它们如何真正咬合进筛选——而不是只出现在报告里。

护栏与准入的关系一句话：**单因子护栏是排序信号，lift 增量检验才是入库裁决。** 详见[因子库与增量准入](factor-library.md)。
...
| 位置 | 行数 | 角色 |
|---|---:|---|
| `validation/` | 240 | **纯统计原语**：DSR、PBO、bootstrap、holdout 切分的教科书实现 |
| `discovery/guardrails.py` | 552 | **阈值单一真源 + 判定逻辑** |
| `discovery/scoring.py` | 690 | **打分与多重检验记账** |
...
`validation/bootstrap.py` `block_bootstrap_ic_ci()`
...
`validation/deflated_sharpe.py` `expected_max_sharpe()` / `deflated_sharpe()`
...
`validation/pbo.py` `compute_pbo(perf_matrix, n_splits=10)`
...
>
> ⚠️ holdout 覆盖不足时必须显式守卫。`DEFAULT_HOLDOUT_MIN_DAYS = 60`（`discovery/guardrails.py`）。样本不够就该说不够，而不是用一个天数极少的 holdout 假装做过样本外。
