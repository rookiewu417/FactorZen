...
| 向前执行与实盘对接 | live-forward 纸面验证 → miniQMT 实盘（分阶段）| 🚧 规划中 |
...
- `attribution/brinson.py` Brinson 单期归因（配置 + 选股两项效应，交互项并入选股）。
...
平台原将「实盘」列为非目标；现升级为**分阶段路线目标**，最终对接 miniQMT/QMT 自动交易。核心设计：定义券商执行接口（`BrokerAdapter`），纸面模拟与真实券商是它的两个后端，同一套账户/归因/续跑逻辑换后端即用——从纸面切到真钱，命令不改、仅换后端。

分阶段（门 = 放行条件，过不了不进下一级）：

| 门 | 目标 | 状态 |
|----|------|------|
| Gate 0 | 有 edge 的策略（过 OOS + PBO/DSR）| ✅ 能力已具备（现有研究链路）|
| Gate 1 | 向前执行引擎：纸面 live-forward（可达性缺口 + 向前衰减两类分歧归因）| 🚧 已设计，待实施 |
| Gate 2 | 小资金真钱·半自动（调仓清单 + 人工回填成交）| ⬜ 规划 |
| Gate 3 | 小资金真钱·自动（miniQMT 下单 + 实盘风控/对账）| ⬜ 规划 |
| Gate 4 | 常态运营（监控/告警/异常处置/扩容）| ⬜ 规划 |

诚实边界：当前处于设计阶段；纸面撮合是模型（开盘价 + 滑点，不建模盘口深度/排队/日内冲击路径），不建模分红送转除权同步；真实下单能力属 Gate 3，尚未实现——未接入前不做任何实盘下单。
...
- 不内置商业行情数据。
- 实盘执行已转为**分阶段路线目标**（见「向前执行与实盘对接」），当前仅到设计/纸面模拟；真实下单（miniQMT）属 Gate 3、尚未实现，未接入前不做任何实盘下单。
