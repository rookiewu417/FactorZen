...
- `risk/style_factors.py` Barra 风格（8 因子：`size` / `value` / `momentum` / `volatility` / `liquidity` / `quality` / `growth` / `leverage`）；`risk/industry_factors.py` 行业（中信一级）暴露。
...
以下方向在现有能力内已预留接口，但尚未实现，可按需扩展：

| 增强项 | 说明 |
|--------|------|
| `fetch_index_weights` 真实指数基准 | 基准管理已预留接口；接入真实沪深 300/中证 500 成分权重 |
