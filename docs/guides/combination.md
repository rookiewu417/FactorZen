...
| `top_bottom_spread` | 分 5 层的多空日均收益差 | 可交易性的粗略代理 |
...
> ⚠️ **`rank_ic_mean` 最高的方法不等于最该上线的方法。** 报告结尾自带这句提醒：需要结合换手、容量、稳健性综合判断；若 ML 未显著胜出，线性方法因更稳健、更可解释而更可运营。**「ML 没赢」本身就是一条有价值的实验记录**，不是失败的实验。
