...
明确不覆盖：盘口撮合、逐笔成交、商业行情数据内置。实盘执行（OMS/EMS）已从非目标升级为分阶段路线目标（向前执行引擎 → miniQMT，见演进计划），当前仅到纸面模拟/设计阶段，真实下单尚未实现。`intraday/` 保留为分钟线研究代码，不是当前路线图主线。
...
| 模拟交易 | 不模拟盘口深度/部分成交/实盘滑点；实盘 OMS 对接为路线目标（Gate 1-4→miniQMT），当前纸面模拟阶段 |
