...
| 目录 | 由谁产生 | 内容 |
|---|---|---|
| `factors/` | `fz factor new`、store sync、eval 写面板 / 评估 run | 每因子：`meta.json` + `factor.py` + **4 列** `factor.parquet`；评估 run 在 `evaluations/` 下；报告在 `reports/` |
...
| `runs/` | 日志等 | 主要为 `runs/logs/`；日频评估大文件不再写 `runs/artifacts/` |
...
### 2.1 `factors/<market>/<name>/evaluations/{run_id}/` —— 单因子评估

`run_id` 缺省格式 `{safe(factor)}_{YYYYmmdd_HHMMSS}`，因子名里的非法字符（`[^A-Za-z0-9_.-]+`）被替换成 `_`。

以 `factors/ashare/momentum_20d/evaluations/momentum_20d_20260717_045904/` 为例：

| 文件 | 体量 | 内容 |
|---|---|---|
| `report.html` | ~450 KB | 单页报告（eval→信号轨 / backtest→交易轨） |
| `manifest.json` | 3.6 KB | 复现清单，见 §3.1 |
| `quality.json` | 1.0 KB | 因子质量指标 |
| `meta.json` | ~1–4 KB | 因子/IC 摘要、方向、`store_panel` 路径等 |
| `walk_forward.json` | 小 | 滚动验证结果（仅 backtest；disabled 时很短） |
| `signal.json` | ~1 KB | 信号层分层统计（仅 eval） |

**不写任何 parquet**（`factor` / `universe` / `ic` / `signal_group_nav` 等）。因子数值面板唯一在：

`workspace/factors/<market>/<name>/factor.parquet`（4 列：`trade_date, ts_code, factor_value, factor_clean`）。

两条轨道产出的 json 集不同：`fz factor eval` 出 `signal.json` 不出 `walk_forward.json`，`fz factor backtest` 反之。

另有跨 run 全局索引 `factors/experiment_index.jsonl`，每 run 追加一行：`run_id, timestamp, factor, universe, start, end, status, manifest_path`。
...
### 2.8 `runs/` —— 日志（评估大文件不再写此处）

```text
runs/logs/                        factor_research.log(+ 按日轮转)
# 旧 runs/artifacts/daily/{factors,results} 已废除：评估产物写 factors/<market>/<name>/evaluations/{run_id}/
# HTML 报告额外镜像到 workspace/factors/reports/（见 §2.9）
```

> ℹ️ **`fz factor eval` / `fz factor backtest` 只写一份评估产物**：  
> - **run 目录**（`factors/<market>/<name>/evaluations/<run_id>/`）用**短名**：`signal.json`、`report.html`、`meta.json` 等（见 §2.1；**无 parquet**）。  
> - **HTML 镜像**：另写 `workspace/factors/reports/daily/{factor}_{start}_{end}[_eval].html`，前端「报告」栏单点可见。  
> - **`factors/`** 资产三件套是因子数值面板唯一落点（4 列 `factor.parquet` + `factor.py` + `meta.json`）；评估 run **不写**评估窗子集覆盖该面板。

qlib 系列因子的报告 HTML 会再分桶：因子名前缀 `qlib_alpha158_` → `qlib158/` 子目录，`qlib_alpha360_` → `qlib360/`（`settings.py` 的 `daily_output_bucket`）。

### 2.9 `workspace/factors/reports/` —— HTML 报告统一收口

所有 HTML 报告集中于此，前端「报告」栏单点可见、可直接在浏览器打开：

```text
workspace/factors/reports/daily/       fz factor eval / backtest 的因子报告（{factor}_{start}_{end}[_eval].html）
workspace/factors/reports/intraday/    日内因子报告
workspace/factors/reports/portfolio_<run_id>.html   fz report portfolio / research run 的组合 dashboard
```

> ℹ️ run 目录内仍各自保留一份短名 `report.html`（run 自包含产物，见 §2.1）；`workspace/factors/reports/` 是跨 run 的集中归档。
...
写入 `factors/<market>/<name>/evaluations/{run_id}/manifest.json`（无 factor 时 `factors/_runs/{run_id}/`）。`run_experiment()` 自管的标准字段全集：
...
`outputs` 只记录 **run 目录内**短名路径（不再双写 `runs/artifacts` / `run_*` 前缀）：

**`fz factor backtest`（交易轨）= 7 键**

- `factor` / `ic` / `quality_report` / `walk_forward_summary` / `universe_snapshot` / `report` / `meta` → 均在评估 run 目录（见 §2.1）

**`fz factor eval`（信号轨）= 8 键**

- `factor` / `ic` / `quality_report` / `universe_snapshot` / `signal` / `signal_group_nav` / `report` / `meta` → 均在评估 run 目录（见 §2.1）

（eval 无 `walk_forward_summary`；backtest 无 `signal` / `signal_group_nav`。见 `pipelines/daily_single.py` 的 `run_factor_eval` / `run_factor_backtest`。）
