...
本页覆盖四个市场的数据来源、落盘位置，以及**单位口径**——后者是本仓两条 P1 级 bug 的根源，任何涉及金额/市值阈值的改动都必须先对照 §3。

产物目录结构见 [产物布局](artifacts.md)，凭据配置见 [环境变量](environment.md)。
...
| Crypto（`crypto`） | Binance Vision 数据湖 | `data/crypto_lake/` | USDT-M 永续，日频 + 分钟 |
| 期货（`futures`） | Tushare（主力连续） | `data/raw/fut_*` | 国内商品期货 |
| 美股（`us`） | — | `data/raw/us_daily/` | **因数据源权限暂缓** |
...
| 接口 | fetch 函数 | 用途 |
|---|---|---|
| `daily` | `fetch_daily:223` | 日线行情 |
| `daily_basic` | `fetch_daily_basic:267` | 每日指标（市值/换手/估值） |
| `stk_mins` | `fetch_minute:304` | 分钟线 |
| `fina_indicator` 等 | `fetch_finance:422` | 财务指标（按公告日 PIT 对齐） |
| `moneyflow` | `fetch_moneyflow:541` | 资金流 |
| `hk_hold` | `fetch_hk_hold:554` | 北向持股 |
| `margin_detail` | `fetch_margin_detail:573` | 两融明细 |
| `stk_holdernumber` | `fetch_stk_holdernumber:694` | 股东户数 |
| `top_list` | `fetch_top_list:852` | 龙虎榜 |
| `stock_basic` | `fetch_stock_basic:927` | 股票列表（`list_status="L,D,P"`，含退市与暂停，PIT 必需） |
| `namechange` | `fetch_namechange:991` | 曾用名（ST 判定） |
| `adj_factor` | `fetch_adj_factor:1052` | 复权因子 |
| `index_daily` | `fetch_index_daily:1078` | 指数日线 |
| `index_member_all` | `fetch_index_member_all:1149` | 指数成分（逐日快照） |
| `trade_cal` | `fetch_trade_cal:1233` | 交易日历 |
...
- 写入方：`dataio/minute_ingest.py:15`，`MINUTE_DATA_TYPE = "minute_1min"`
- 消费方统一走 `load_parquet("minute_1min", ...)`：`intraday/audit.py:305`、`intraday/features/engine.py:566`、`intraday/bars_cache.py:286`、`discovery/intraday_expr.py:245`
...
| 数据 | 字段 | 单位 | 换算到元 | 现场 |
|---|---|---|---|---|
| A 股日线 | `daily.amount` | **千元** | ×1e3 | `daily/data/flows.py:50` |
| A 股每日指标 | `daily_basic.total_mv` | **万元** | ×1e4 | — |
| A 股每日指标 | `daily_basic.circ_mv` | **万元** | ×1e4 | `daily/data/flows.py:49` |
| 龙虎榜 | `top_list.net_amount` | **万元** | ×1e4 | `daily/data/flows.py:51`、`core/loader.py:765` |
| 龙虎榜 | `top_list.amount` | **千元** | ×1e3 | 同上 |
| 资金流 | `moneyflow.net_mf_amount` | **万元** | ×1e4 | `daily/data/flows.py:40` |
| 两融 | `margin_detail.rzye` / `rzmre` | **元** | ×1 | `daily/data/flows.py:25,96` |
| 两融 | `margin_detail.rqyl` | 股 | — | `daily/data/flows.py:44` |
| **期货** | `fut_daily.amount` | **万元** | ×1e4 | `markets/futures/factors.py:11` |
| 期货 | `fut_daily.vol` / `oi` | 手（lots） | — | 同上 |
| A 股分钟 | `vol` | 股 | — | `intraday/sessions.py:11` |
| A 股分钟 | `amount` | **元** | ×1 | 同上 |
| A 股日线 | `vol` | 手 | — | 同上 |

> ⚠️ **期货的 `amount` 是万元，与 A 股日线的千元不同。** 跨市场复用金额逻辑时这是第一个会踩的地方。`markets/futures/factors.py:12` 还提示 `vwap = amount/vol = 万元/手`，是品种内的活跃度代理，**跨品种不可比**。
...
```python
_CIRC_MV_TO_YUAN = 1e4    # flows.py:49
_AMOUNT_TO_YUAN  = 1e3    # flows.py:50
_NET_AMOUNT_TO_YUAN = 1e4 # flows.py:51  top_list net_amount 万元
```
...
| 派生量 | 公式 | 现场 |
|---|---|---|
| `margin_ratio` | `rzye(t') / (circ_mv(t') × 1e4)` | `flows.py:122` |
| 龙虎榜净买占比 | `(net_amount × 1e4) / (amount × 1e3)` | `flows.py:164` |

连喂给 LLM 的 prompt 里都显式带上了单位说明（`llm/generation.py:160`：「net_amount 万元、amount 千元，比前统一到元」），避免生成的表达式在单位上出错。

### 3.3 踩过的真 bug：流动性门槛

`core/universe.py:1187-1188` 保留着事故现场的注释：
...
**真实生效的流动性门槛是 `min_amount = 10_000_000`（1000 万元）**，定义在 `core/universe.py:724` 与 `core/universe.py:1160`。

> ⚠️ `config/constants.py:44` 定义了 `MIN_MARKET_CAP_CNY = 3e8`（3 亿），但它在 `src/` 里**没有任何消费方**——常量已声明、**未接线**。不要把它当成生效的默认市值过滤。
...
| 用途 | 位置 |
|---|---|
| Barra Size 因子 `ln(total_mv)` | `risk/style_factors.py:100` |
| 市值中性化用 `log(total_mv)` | `daily/preprocessing/neutralizer.py:182-188` |
| 内置 size 因子 | `builtin_factors/daily/size.py:18-19` |
...
`markets/crypto/profile.py:37-38` 的解析规则：
...
| 其它 | `ValueError("未知 source: ...,支持 'lake' / 'ccxt'")`（`profile.py:44`） |

两条守卫：

- **湖为空**：`klines_1m` 目录不存在 → 报错「crypto 数据湖为空(...)：先运行 `fz data crypto backfill`」（`lake_provider.py:31`）。
- **ccxt 只支持日频**：intraday 调用抛 `ValueError("CryptoDataProvider(ccxt) 仅支持 daily;intraday 请用数据湖 provider")`（`provider.py:105,149`）。分钟级研究必须走湖。
...
`discovery/python_factor.py:91-104` 是本仓的参考实现，注释里直接写着「缓存键完整性是本仓 P1 教训」：
...
`impl_sha` 取因子实现源文件的 sha1（`_impl_source_sha`，`python_factor.py:63`）。**动态类（`type()` 生成）取不到源码 → 返回 `None` → 整条路径跳过缓存**，宁可慢也不毒化。
...
| 位置 | 说明 |
|---|---|
| `core/loader.py:248` | 全市场：交易日历覆盖审计，只拉缺失交易日 |
| `core/loader.py:576` | 日频按缺失交易日市场级拉取 + 缓存 |
| `core/loader.py:1099` | 交易日历覆盖审计替代分区存在启发式 |

### 5.3 事件类数据的「诚实缺测」

龙虎榜这类事件数据不能简单 fill 0——「没拉到」和「确定没上榜」是两回事。`daily/data/flows.py:155-170` 的规则：
