...
| 变量 | 默认 / 示例 | 必需性 | 消费点 |
|---|---|---|---|
| `TUSHARE_TOKEN` | `your_tushare_token_here` | 真实取数**必需** | `config/tushare_config.py` |
| `TUSHARE_POINTS` | `2000` | 可选（示例中被注释） | `config/tushare_config.py` |
| `TUSHARE_MAX_RPS` | `5` | 可选（示例中被注释） | `config/tushare_config.py` |
| `FACTORZEN_LLM_ENABLED` | `false` | 见 §3 | `llm/config.py` |
| `FACTORZEN_LLM_BASE_URL` | `https://www.aiping.cn/api/v1` | LLM 功能必需 | `llm/config.py` |
| `FACTORZEN_LLM_API_KEY` | `your_llm_api_key_here` | LLM 功能必需 | `llm/config.py` |
| `FACTORZEN_LLM_MODEL` | `DeepSeek-V4-Pro` | LLM 功能必需 | `llm/config.py` |
| `FACTORZEN_LLM_TIMEOUT_SECONDS` | `30` | 可选 | `llm/config.py` |
| `FACTORZEN_LLM_MAX_TOKENS` | `700` | 可选 | `llm/config.py` |
| `FACTORZEN_LLM_THINKING` | `false`（示例中被注释） | 可选 | `llm/config.py` |
| `FACTORZEN_LLM_PROVIDER` | `DeepSeek` | 可选 | `llm/config.py` |

### 1.2 代码真实读取、但 `.env.example` 未列的变量

| 变量 | 默认 | 消费点 | 用途 |
|---|---|---|---|
| `FACTORZEN_LLM_PROFILE` | 未设 | `llm/config.py` | profile 切换，见 §2 |
| `FACTORZEN_LLM_FLAVOR` | `aiping` | `llm/config.py` | 上游适配风格，见 §2.2 |
| `FACTORZEN_LLM_STREAM` | 按 flavor 推导 | `llm/config.py` | 流式开关，见 §2.2 |
| `FACTORZEN_LLM_MAX_RETRIES` | `3` | `llm/config.py` | 交给 OpenAI SDK 的重试次数 |
| `FACTORZEN_POOL_SUBPROC` | 未设 | `cli/main.py` | `=1` 时池构建放子进程，退出全额归还内存；等效同名 CLI 旗标（`cli/parser.py` help 明说） |
| `FACTORZEN_NOTIFY_WEBHOOK` | — | `ops/config.py` | 无人值守运营通知 webhook。注意配置里的字段 `notify_url_env` **存的是变量名**，不是值 |
| `QLIB_PROVIDER_URI` | `~/.qlib/qlib_data/cn_data` | `builtin_factors/qlib/handler.py`、`core/data_ensure.py` | qlib 数据目录 |
| `QLIB_INSTRUMENTS` | `csi500` | `builtin_factors/qlib/handler.py` | qlib universe |
| `QLIB_KERNELS` | `1` | `builtin_factors/qlib/handler.py` | qlib 并行核数 |
| `QLIB_JOBLIB_BACKEND` | `threading` | `builtin_factors/qlib/handler.py` | qlib joblib 后端 |
| `TUSHARE_API_URL` | `http://api.tushare.pro` | `tools/download_tushare_lake.py` | 湖下载脚本的 API 端点 |
| `TUSHARE_LAKE_MAX_PER_MIN` | `140` | `tools/download_tushare_lake.py` | 每分钟请求上限 |
| `TUSHARE_LAKE_WORKERS` | `6` | `tools/download_tushare_lake.py` | 并发 worker 数 |
...
`STREAM` 未显式设置时按 flavor 取缺省（`llm/config.py` 的 `LLMConfig.stream_enabled`）：
...
`enabled` 本身是两级与运算（`llm/config.py`）：
...
| 场景 | 缺配置时的行为 |
|---|---|
| `fz mine agent`、`fz mine team` | **直接报错退出**：`RuntimeError("LLM 未配置：设置 .env 的 FACTORZEN_LLM_* 或注入 llm_fn")`（`pipelines/factor_mine_agent.py`、`pipelines/factor_mine_team.py`） |
...
| `llm/config.py::_read_env_file` | `load_llm_config` 调用时 | **只收集 `FACTORZEN_LLM_` 前缀键** | 真实环境变量优先（`os.getenv(name) or file_values.get(name)`，`llm/config.py`） |
...
- **用 `utf-8-sig` 打开以容忍 BOM**。历史 bug：用 `utf-8` 会把 BOM 读进首行，让 `TUSHARE_TOKEN` 变成 `﻿TUSHARE_TOKEN` 而**静默失效**（`config/tushare_config.py` 的 `_load_dotenv`）。
...
- 真正需要取数时 `ensure_token()` 才抛 `RuntimeError`，错误信息同时给出 Windows `set` 与 `.env` 两种写法（`config/tushare_config.py`）。
