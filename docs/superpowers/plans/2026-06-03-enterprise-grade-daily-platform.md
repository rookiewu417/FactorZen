...
- [x] `reports/_charts.py` —— 全部 `_make_*_chart` + matplotlib 设置 + 图表辅助(705 行)
- [x] `reports/_strategy.py` —— 策略命名/口径/暴露/约束/参数 + 交易&执行摘要(452 行,ast 精确抽取)
- [x] `reports/_summaries.py` —— `_build_*_summary` / `_build_*_notice` / 质量·regime·status 显示助手(601 行)
