...
_SIGNAL_FAMILIES = (
    ...
    "**股东户数**（holder_num/环比，ann_date PIT，筹码集中度边际）、"
    "**资金流**（主力净流入等）、"
    "**两融/杠杆情绪**（融资余额占比/融资买入参与度/融券余量等，T+1 lag 已内置，以当前可用叶子为准）、"
    "**龙虎榜**（净买入占比/是否上榜，盘后披露 lag(1)，未上榜=0）、"
    "**业绩预告/快报**（fc_type_score/fc_surprise/fc_flag/express_yoy，ann_date 盘后→t+1 生效，20 日窗 fill-0）。"
