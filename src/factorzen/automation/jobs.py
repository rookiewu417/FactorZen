...
from factorzen.core.loader import fetch_daily, fetch_index_daily
...
def job_evaluate(date: str, factor_name: str) -> None:
    ...
    通过 subprocess 调用包内 pipeline 以避免循环导入。
    ...
    try:
        cmd = [
            ...
            "python",
            "-m",
            "factorzen.pipelines.daily_single",
            "--factor",
            factor_name,
            "--start",
            date,
...
def job_generate_report(date: str, factor_name: str) -> None:
    ...
    try:
        cmd = [
            ...
            "python",
            "-m",
            "factorzen.pipelines.generate_report",
            "--factor",
            factor_name,
            "--start",
            date,
