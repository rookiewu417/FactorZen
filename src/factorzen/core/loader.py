...
def _normalize_holder_frame(parts: list[pl.DataFrame]) -> pl.DataFrame:
    ...
    # 源数据垃圾行防御（实测 Tushare 返回 end_date=1900-09-08/2053-06-26/2028-03-10、
    # null 的坏行，且 holder_num 同为 null）：null end_date 会崩分区路径构造
    # （month.zfill）；**未来的报告期末逻辑不可能**，且 pit_align 取最大 end_date,
    # 一条未来垃圾行会永久霸占该股最新期——上界必须是抓取时点。过滤并记数,不静默。
    import datetime as _dt
    before = df.height
    df = df.filter(
        pl.col("end_date").is_not_null()
        & (pl.col("end_date") >= _dt.date(2000, 1, 1))
        & (pl.col("end_date") <= _dt.date.today())
    )
    if df.height < before:
