...
def test_probation_cap_core_suite(tmp_path):
    """decision=active + 默认 allow_active=False → status=probation，裁决保留。；decision=active + allow_active=True → status=active（现行为）。；decision=probation → 不受 cap 影响，admission_decision 仍为 probation。；已 forward-confirmed 的 lift active + 复测 pass → 保持 active，确认字段不洗。；已 forward-confirmed 的 lift active + 复测统计降级 → 允许降到 probation。"""
    # -- 原 test_active_decision_capped_to_probation_by_default --
    def _section_0_test_active_decision_capped_to_probation_by_default(tmp_path):
    ...
    _section_0_test_active_decision_capped_to_probation_by_default(_tp0)

    # -- 原 test_allow_active_true_writes_active --
    def _section_1_test_allow_active_true_writes_active(tmp_path):
        from factorzen.discovery.factor_library import load_library, upsert_lift_admissions
    ...
    _section_1_test_allow_active_true_writes_active(_tp1)

    # -- 原 test_probation_decision_unaffected_by_cap --
    def _section_2_test_probation_decision_unaffected_by_cap(tmp_path):
        from factorzen.discovery.factor_library import load_library, upsert_lift_admissions
    ...
    _section_2_test_probation_decision_unaffected_by_cap(_tp2)

    # -- 原 test_confirmed_lift_active_kept_on_active_retest --
    def _section_3_test_confirmed_lift_active_kept_on_active_retest(tmp_path):
    ...
    _section_3_test_confirmed_lift_active_kept_on_active_retest(_tp3)

    # -- 原 test_confirmed_lift_active_demotes_on_probation_decision --
    def _section_4_test_confirmed_lift_active_demotes_on_probation_decision(tmp_path):
        from factorzen.discovery.factor_library import (
            FactorRecord,
            _save_library,
            load_library,
    ...
    _section_4_test_confirmed_lift_active_demotes_on_probation_decision(_tp4)
