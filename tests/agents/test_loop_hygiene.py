...
def test_parallel_llm_parity_suite(tmp_path, monkeypatch):
    """同 seed 下 llm_workers=4 与 =1 的 attempts 表达式集合与顺序完全一致。；llm_workers=4 同 seed 跑两次：人为延迟让 B 先完成，attempts 仍逐位一致。；某假设链 write 抛 LLMClientError → 该轮按 LLM 失败计（与串行一致）。；llm_workers=1 必须不实例化 ThreadPoolExecutor。"""
    # -- 原 test_parallel_serial_parity_same_seed --
    def _section_0_test_parallel_serial_parity_same_seed(tmp_path):
    ...
    _section_0_test_parallel_serial_parity_same_seed(_tp0)

    # -- 原 test_parallel_deterministic_despite_completion_order --
    def _section_1_test_parallel_deterministic_despite_completion_order(tmp_path):
        daily = _mock_daily__llm_parallel()
    ...
    # -- 原 test_parallel_llm_error_counts_as_round_failure --
    def _section_2_test_parallel_llm_error_counts_as_round_failure(tmp_path):
        lock = threading.Lock()
...
def test_llm_workers_cli_suite(monkeypatch, tmp_path):
    """test_parser_mine_team_llm_workers_default_is_four；test_cmd_mine_team_forwards_llm_workers；pipeline 透传 llm_workers 到 orchestrator，并写入 manifest params。；API 缺省 llm_workers=1（零回归）。"""
    # -- 原 test_parser_mine_team_llm_workers_default_is_four --
    def _section_0_test_parser_mine_team_llm_workers_default_is_four():
...
def test_agent_loop_resilience_suite():
    """某轮 LLM 不可用 → 跳过该轮继续，而非冒泡崩掉整个 session。；LLM 持续不可用时提前终止，不空转跑满 n_rounds。；失败计数器必须在成功轮重置——否则零散的抖动会被累计成「持续不可用」。；只吞 LLMClientError。别的异常（代码 bug、磁盘满）必须冒泡，不许静默吞掉。"""
    # -- 原 test_agent_loop_survives_llm_error_and_continues --
    def _section_0_test_agent_loop_survives_llm_error_and_continues():
    ...
    _section_0_test_agent_loop_survives_llm_error_and_continues()

    # -- 原 test_agent_loop_aborts_after_consecutive_llm_failures --
    def _section_1_test_agent_loop_aborts_after_consecutive_llm_failures():
    ...
    _section_1_test_agent_loop_aborts_after_consecutive_llm_failures()

    # -- 原 test_consecutive_failure_counter_resets_on_success --
    def _section_2_test_consecutive_failure_counter_resets_on_success():
    ...
    _section_2_test_consecutive_failure_counter_resets_on_success()

    # -- 原 test_non_llm_exception_still_propagates --
    def _section_3_test_non_llm_exception_still_propagates():
    ...
    _section_3_test_non_llm_exception_still_propagates()


# ── 增量落盘 ────────────────────────────────────────────────────────────────


def test_on_round_end_manifest_suite(tmp_path):
    """test_on_round_end_called_after_each_successful_round；失败轮没有产出，不该触发落盘回调。；不可恢复的崩溃发生在第 3 轮 → 前两轮的成果必须已经落盘。；test_completed_run_marks_manifest_not_partial"""
    # -- 原 test_on_round_end_called_after_each_successful_round --
    def _section_0_test_on_round_end_called_after_each_successful_round():
        seen: list[int] = []
    ...
    _section_0_test_on_round_end_called_after_each_successful_round()

    # -- 原 test_on_round_end_not_called_for_failed_round --
    def _section_1_test_on_round_end_not_called_for_failed_round():
        seen: list[int] = []
    ...
    _section_1_test_on_round_end_not_called_for_failed_round()

    # -- 原 test_manifest_survives_mid_loop_crash --
    def _section_2_test_manifest_survives_mid_loop_crash(tmp_path):
    ...
    _tp2 = tmp_path / "_s2"
    _tp2.mkdir(exist_ok=True)
    _section_2_test_manifest_survives_mid_loop_crash(_tp2)

    # -- 原 test_completed_run_marks_manifest_not_partial --
    def _section_3_test_completed_run_marks_manifest_not_partial(tmp_path):
        from factorzen.pipelines.factor_mine_agent import run_agent_mine
    ...
    _tp3 = tmp_path / "_s3"
    _tp3.mkdir(exist_ok=True)
    _section_3_test_completed_run_marks_manifest_not_partial(_tp3)
...
def test_team_loop_resilience_suite(tmp_path):
    """test_team_loop_survives_llm_error_and_continues；test_team_loop_aborts_after_consecutive_llm_failures；test_team_manifest_survives_mid_loop_crash"""
    # -- 原 test_team_loop_survives_llm_error_and_continues --
    def _section_0_test_team_loop_survives_llm_error_and_continues(tmp_path):
        from factorzen.agents.team_orchestrator import run_team_agent
    ...
    _tp0 = tmp_path / "_s0"
    _tp0.mkdir(exist_ok=True)
    _section_0_test_team_loop_survives_llm_error_and_continues(_tp0)

    # -- 原 test_team_loop_aborts_after_consecutive_llm_failures --
    def _section_1_test_team_loop_aborts_after_consecutive_llm_failures(tmp_path):
        from factorzen.agents.team_orchestrator import run_team_agent
    ...
    _tp1 = tmp_path / "_s1"
    _tp1.mkdir(exist_ok=True)
    _section_1_test_team_loop_aborts_after_consecutive_llm_failures(_tp1)

    # -- 原 test_team_manifest_survives_mid_loop_crash --
    def _section_2_test_team_manifest_survives_mid_loop_crash(tmp_path):
    ...
    _tp2 = tmp_path / "_s2"
    _tp2.mkdir(exist_ok=True)
    _section_2_test_team_manifest_survives_mid_loop_crash(_tp2)
...
def test_eval_hygiene_critic_compile_suite(caplog, tmp_path):
    """一个候选的 holdout 求值失败必须留下日志，而不是静默 continue。；真实 LLM 常返回 markdown 围栏。单 Agent 与 team 的 Critic 必须给出同一裁决。；完全无法解析时仍 fail-open（不误杀），但那是**解析失败**，不是格式差异。；坏表达式不进 index → `seen_expressions()` 看不到 → 跨 session 反复生成同一语法坑。；`known_invalid` 的语义是「能编译但无效」。语法坑 ic_train=None → 排序键 0.0 会排最前，；编译失败记录的 ir_train=None，必须被 DeflationBasis 剔除——否则污染 N 与经验方差。；`heal_rounds=0` 时 heal 的去重不生效；多个 task 翻译出同一表达式 → 不得评估两次。"""
    # -- 原 test_node_guardrails_logs_when_a_candidate_blows_up --
    def _section_0_test_node_guardrails_logs_when_a_candidate_blows_up(caplog):
    ...
    _section_0_test_node_guardrails_logs_when_a_candidate_blows_up(caplog)

    # -- 原 test_node_critic_parses_fenced_json_like_team_critic_does --
    def _section_1_test_node_critic_parses_fenced_json_like_team_critic_does():
    ...
    caplog.clear()
    _section_1_test_node_critic_parses_fenced_json_like_team_critic_does()

    # -- 原 test_node_critic_still_defaults_to_keep_on_garbage --
    def _section_2_test_node_critic_still_defaults_to_keep_on_garbage():
        from factorzen.agents.nodes import node_critic
        from factorzen.agents.state import AgentState, AttemptRecord
    ...
    caplog.clear()
    _section_2_test_node_critic_still_defaults_to_keep_on_garbage()

    # -- 原 test_record_persists_compile_failures_so_they_are_not_retried --
    def _section_3_test_record_persists_compile_failures_so_they_are_not_retried(tmp_path):
    ...
    _tp3 = tmp_path / "_s3"
    _tp3.mkdir(exist_ok=True)
    _section_3_test_record_persists_compile_failures_so_they_are_not_retried(_tp3)

    # -- 原 test_known_invalid_excludes_compile_failures --
    def _section_4_test_known_invalid_excludes_compile_failures(tmp_path):
        from factorzen.agents.experiment_index import ExperimentIndex
        from factorzen.agents.roles.librarian import record
        from factorzen.agents.state import AttemptRecord
    ...
    _section_4_test_known_invalid_excludes_compile_failures(_tp4)

    # -- 原 test_compile_failures_never_enter_the_deflation_pool --
    def _section_5_test_compile_failures_never_enter_the_deflation_pool(tmp_path):
    ...
    _section_5_test_compile_failures_never_enter_the_deflation_pool(_tp5)

    # -- 原 test_team_evaluate_deduplicates_within_the_batch --
    def _section_6_test_team_evaluate_deduplicates_within_the_batch():
        from factorzen.agents.state import AgentState
    ...
    caplog.clear()
    _section_6_test_team_evaluate_deduplicates_within_the_batch()
...
def test_heal_unknown_op_suite():
    """ts_delta 等未知算子不触发 revise_from_error，计数=1。；普通语法错（非未知算子）仍进 heal 调 LLM。；drop_unknown_ops=False 时未知算子仍进 heal（兼容开关）。"""
    # -- 原 test_heal_drops_unknown_op_no_llm --
    def _section_0_test_heal_drops_unknown_op_no_llm():
        calls = {"n": 0}
    ...
    _section_0_test_heal_drops_unknown_op_no_llm()

    # -- 原 test_heal_syntax_error_still_enters_heal --
    def _section_1_test_heal_syntax_error_still_enters_heal():
        calls = {"n": 0}
    ...
    _section_1_test_heal_syntax_error_still_enters_heal()

    # -- 原 test_heal_drop_unknown_ops_false_restores_old --
    def _section_2_test_heal_drop_unknown_ops_false_restores_old():
        calls = {"n": 0}
    ...
    _section_2_test_heal_drop_unknown_ops_false_restores_old()


# ── W5b ──────────────────────────────────────────────────────────────────────

def test_clamp_window_suite():
    """504 窗 + 预算 400 → 钳到 400。；预算充足 → 不动。；无窗口字面量（纯截面）→ 原样。；test_clamp_window_parse_fail_passthrough；嵌套：仅超 cap 的窗口被钳。"""
    # -- 原 test_clamp_window_over_budget --
    def _section_0_test_clamp_window_over_budget():
    ...
    _section_0_test_clamp_window_over_budget()

    # -- 原 test_clamp_window_budget_sufficient_unchanged --
    def _section_1_test_clamp_window_budget_sufficient_unchanged():
        expr = "ts_mean(amount, 20)"
        out, did = clamp_window_literals(expr, {"amount": 400}, None)
        assert did is False
        assert out == expr

    _section_1_test_clamp_window_budget_sufficient_unchanged()

    # -- 原 test_clamp_window_no_window_literal --
    def _section_2_test_clamp_window_no_window_literal():
        expr = "rank(amount)"
        out, did = clamp_window_literals(expr, {"amount": 400}, None)
        assert did is False
        assert out == expr

    _section_2_test_clamp_window_no_window_literal()

    # -- 原 test_clamp_window_parse_fail_passthrough --
    def _section_3_test_clamp_window_parse_fail_passthrough():
        expr = "not_parseable!!!"
        out, did = clamp_window_literals(expr, {"amount": 400}, None)
        assert out == expr and did is False

    _section_3_test_clamp_window_parse_fail_passthrough()

    # -- 原 test_clamp_nested_windows --
    def _section_4_test_clamp_nested_windows():
        out, did = clamp_window_literals(
    ...
    _section_4_test_clamp_nested_windows()
...
def test_empty_round_critic_suite(tmp_path):
    """new_cands=[] 时 critic 零调用，verdict=revise_hypothesis，critic_skipped=True。；有新候选时 critic 仍被调用。"""
    # -- 原 test_empty_round_skips_critic_llm --
    def _section_0_test_empty_round_skips_critic_llm(tmp_path):
        from factorzen.agents.team_orchestrator import run_team_agent
    ...
    _tp0 = tmp_path / "_s0"
    _tp0.mkdir(exist_ok=True)
    _section_0_test_empty_round_skips_critic_llm(_tp0)

    # -- 原 test_nonempty_round_still_calls_critic --
    def _section_1_test_nonempty_round_still_calls_critic(tmp_path):
        from factorzen.agents.team_orchestrator import run_team_agent
    ...
    _tp1 = tmp_path / "_s1"
    _tp1.mkdir(exist_ok=True)
    _section_1_test_nonempty_round_still_calls_critic(_tp1)
...
def test_health_check_diag_suite(monkeypatch):
    """test_healthy_expression_reports_no_diagnosis；全 null = 静默失明，必须被抓出来（PR #61 那类 bug 的兜底）。；健康表达式在极严阈值下也会被判不健康 —— 证明判据真的是比例而非硬编码。；test_parse_error_is_diagnosed；求值抛异常 → 诊断带上异常类型与消息（供 LLM 修正）。"""
    # -- 原 test_healthy_expression_reports_no_diagnosis --
    def _section_0_test_healthy_expression_reports_no_diagnosis():
        check = make_health_check(_mock_daily__health_check())
        assert check(_HEALTHY) is None

    _section_0_test_healthy_expression_reports_no_diagnosis()

    # -- 原 test_all_null_factor_is_diagnosed --
    def _section_1_test_all_null_factor_is_diagnosed():
        check = make_health_check(_mock_daily__health_check())
    ...
    def _section_2_test_null_ratio_threshold_is_configurable_and_respected():
    ...
    def _section_3_test_parse_error_is_diagnosed():
        check = make_health_check(_mock_daily__health_check())
...
def test_heal_health_integration_suite():
    """parse 通过但全 null → 诊断回灌 LLM → 换成健康表达式。；LLM 持续产全 null → max_rounds 耗尽后丢弃，不死循环、不放行病态因子。；零额外成本不变量：健康表达式不调用 LLM。；health_check=None（默认）→ 只查 parse，全 null 表达式照旧放行（既有行为）。"""
    # -- 原 test_heal_revises_expression_that_parses_but_evaluates_all_null --
    def _section_0_test_heal_revises_expression_that_parses_but_evaluates_all_null():
        prompts: list[str] = []
    ...
    _section_0_test_heal_revises_expression_that_parses_but_evaluates_all_null()

    # -- 原 test_heal_gives_up_when_llm_keeps_producing_unhealthy_expressions --
    def _section_1_test_heal_gives_up_when_llm_keeps_producing_unhealthy_expressions():
        def fake(_msgs):
    ...
    _section_1_test_heal_gives_up_when_llm_keeps_producing_unhealthy_expressions()

    # -- 原 test_healthy_expression_never_triggers_llm_even_with_health_check --
    def _section_2_test_healthy_expression_never_triggers_llm_even_with_health_check():
        def fake(_msgs):
    ...
    _section_2_test_healthy_expression_never_triggers_llm_even_with_health_check()

    # -- 原 test_no_health_check_is_zero_regression --
    def _section_3_test_no_health_check_is_zero_regression():
        def fake(_msgs):
    ...
    _section_3_test_no_health_check_is_zero_regression()
