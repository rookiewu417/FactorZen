...
class TestRetrySuccess:
    def test_retry_success_suite(self):
        """成功返回非空结果时只调用一次。；test_passes_args_and_kwargs"""
        # -- 原 test_returns_immediately_on_success --
...
class TestRetryNetworkError:
    def test_retry_network_suite(self):
        """网络/超时类错误重试 MAX_RETRIES 次，总计 MAX_RETRIES+1 次调用。；每次重试前 sleep 一次，共 MAX_RETRIES 次。；前几次失败、后面成功时只抛出一次异常就恢复。"""
        # -- 原 test_retries_on_network_error --
        ...
        # -- 原 test_sleep_called_max_retries_times --
        ...
        # -- 原 test_succeeds_on_second_attempt --
        mock_func = MagicMock(
...
class TestRetryPermissionError:
    @pytest.mark.parametrize("msg", ["token invalid",
        "权限不足",])
...
class TestRetryRateLimit:
    def test_retry_rate_limit_suite(self):
        """频率超限错误等待 62 秒。；消息含'频率'（不含'超限'）同样等待 62 秒。"""
        # -- 原 test_rate_limit_waits_62s --
        ...
        # -- 原 test_rate_limit_keyword_triggers_62s --
...
class TestRetryEmptyResult:
    def test_retry_empty_suite(self):
        """返回空 DataFrame 等同于无数据，触发重试直到耗尽。；返回 None 等同于无数据，触发重试。"""
        # -- 原 test_empty_dataframe_triggers_retry --
        ...
        # -- 原 test_none_result_triggers_retry --
...
class TestFetchStockBasicCache:
    def test_stock_basic_cache_suite(self, tmp_path):
        """缓存文件刚写入（mtime 几乎为 now）→ 跳过 API 调用。；缓存文件不存在 → 调用 API，并将结果写入缓存文件。；缓存文件过期（模拟 mtime 是 8 天前）→ 调用 API。；API 返回 pandas DataFrame，fetch_stock_basic 输出必须是 polars。"""
        # -- 原 test_fresh_cache_skips_api --
        _tp0 = tmp_path / "_s0"
        _tp0.mkdir(exist_ok=True)
        cache_file = _tp0 / "stock_basic_L_D_P.parquet"
        ...
        with (
            patch.object(loader_module, "init_tushare", return_value=mock_pro),
            patch.object(loader_module, "DATA_CACHE", _tp0),
        ):
        ...
        # -- 原 test_missing_cache_calls_api_and_writes --
        _tp1 = tmp_path / "_s1"
        ...
        with (
            patch.object(loader_module, "init_tushare", return_value=mock_pro),
            patch.object(loader_module, "DATA_CACHE", _tp1),
            patch.object(loader_module, "_rate_limit"),
        ):
        ...
        assert (_tp1 / "stock_basic_L.parquet").exists()

        # -- 原 test_stale_cache_calls_api --
        _tp2 = tmp_path / "_s2"
        _tp2.mkdir(exist_ok=True)
        import time

        cache_file = _tp2 / "stock_basic_L_D_P.parquet"
        ...
        with (
            patch.object(loader_module, "init_tushare", return_value=mock_pro),
            patch.object(loader_module, "DATA_CACHE", _tp2),
            patch.object(loader_module, "_rate_limit"),
        ):
            result = fetch_stock_basic()
        ...
        # -- 原 test_pandas_to_polars_conversion --
        _tp3 = tmp_path / "_s3"
        ...
        with (
            patch.object(loader_module, "init_tushare", return_value=mock_pro),
            patch.object(loader_module, "DATA_CACHE", _tp3),
            patch.object(loader_module, "_rate_limit"),
        ):
            result = fetch_stock_basic(list_status="L")
...
class TestFetchFinanceBatchCount:
    def test_finance_batch_count_suite(self):
        """120 只股票 × 2 季度 → ceil(120/50)=3 批/季 × 2 = 6 次 API 调用。；50 只股票恰好一批，1 季度 → 1 次 API 调用。；已缓存分区 → 当季所有批次都跳过，API 不被调用。"""
        # -- 原 test_120_stocks_2_quarters_6_calls --
        ...
        # -- 原 test_50_stocks_1_quarter_1_call --
        ...
        # -- 原 test_cached_quarter_skips_all_batches --
        codes = [f"{i:06d}.SZ" for i in range(120)]
        fin_api_mock = MagicMock(return_value=_pd_finance())
        mock_pro = MagicMock()
        mock_pro.fina_indicator = fin_api_mock
...
class TestFetchNamechangeCache:
    def test_namechange_cache_suite(self, tmp_path):
        """缓存文件刚写入（mtime 几乎为 now）→ 跳过 API 调用。；缓存不存在 → 调用 API。；缓存文件过期（模拟 mtime 是 8 天前）→ 调用 API。；API 返回 pandas，fetch_namechange 输出必须是 polars 且日期列已转换为 pl.Date。；拉取失败且无可用本地缓存 → 向上抛出异常，由调用方决定如何降级。；拉取失败但本地存在（即使过期的）缓存 → 回退读取缓存，不向上抛异常。；_retry 返回空结果且无缓存 → 返回空 DataFrame，不抛异常。"""
        # -- 原 test_fresh_cache_skips_api --
        _tp0 = tmp_path / "_s0"
        _tp0.mkdir(exist_ok=True)
        cache_file = _tp0 / "namechange.parquet"
        ...
        # -- 原 test_missing_cache_calls_api_without_date_params_and_writes --
        _tp1 = tmp_path / "_s1"
        _tp1.mkdir(exist_ok=True)
        mock_pro = MagicMock()
        ...
        assert (_tp1 / "namechange.parquet").exists()
        ...
        cache_file = _tp2 / "namechange.parquet"
        ...
        # -- 原 test_pandas_to_polars_date_casting --
        _tp3 = tmp_path / "_s3"
        _tp3.mkdir(exist_ok=True)
        mock_pro = MagicMock()
        mock_pro.namechange.return_value = _pd_namechange(n=2)
        ...
        # -- 原 test_fetch_failure_no_cache_raises --
        _tp4 = tmp_path / "_s4"
        ...
        with (
            patch.object(loader_module, "init_tushare", return_value=mock_pro),
            patch.object(loader_module, "DATA_CACHE", _tp4),
            patch.object(loader_module, "_rate_limit"),
            patch("time.sleep"),
        ...
        # -- 原 test_fetch_failure_with_stale_cache_falls_back --
        _tp5 = tmp_path / "_s5"
        _tp5.mkdir(exist_ok=True)
        import os
        import time

        cache_file = _tp5 / "namechange.parquet"
        fake = pl.DataFrame({"ts_code": ["000001.SZ"], "change_reason": ["ST"]})
        fake.write_parquet(cache_file)
        ...
        with (
            patch.object(loader_module, "init_tushare", return_value=mock_pro),
            patch.object(loader_module, "DATA_CACHE", _tp5),
            patch.object(loader_module, "_rate_limit"),
            patch("time.sleep"),
        ):
            result = fetch_namechange()
        ...
        # -- 原 test_empty_result_no_cache_returns_empty_df --
        _tp6 = tmp_path / "_s6"
        ...
        with (
            patch.object(loader_module, "init_tushare", return_value=mock_pro),
            patch.object(loader_module, "DATA_CACHE", _tp6),
...
def test_quality_report_suite():
    """test_quality_report_records_coverage_and_warnings；test_quality_report_rejects_duplicate_daily_keys；test_quality_report_rejects_empty_clean_factor"""
    # -- 原 test_quality_report_records_coverage_and_warnings --
    def _section_0_test_quality_report_records_coverage_and_warnings():
    ...
    _section_0_test_quality_report_records_coverage_and_warnings()

    # -- 原 test_quality_report_rejects_duplicate_daily_keys --
    def _section_1_test_quality_report_rejects_duplicate_daily_keys():
    ...
    _section_1_test_quality_report_rejects_duplicate_daily_keys()

    # -- 原 test_quality_report_rejects_empty_clean_factor --
    def _section_2_test_quality_report_rejects_empty_clean_factor():
        from factorzen.core.data_quality import QualityCheckError, build_daily_quality_report
    ...
    _section_2_test_quality_report_rejects_empty_clean_factor()
...
class TestLoadFailure:
    def test_data_audit_load_fail_suite(self):
        """test_scan_exception_returns_error；test_empty_dataframe_returns_error"""
        # -- 原 test_scan_exception_returns_error --
        ...
        # -- 原 test_empty_dataframe_returns_error --
        with (
...
class TestDailyDateCoverage:
    ...
    def test_daily_date_coverage_suite(self):
        """test_no_gaps_ok；test_missing_date_warning；test_missing_dates_capped_at_20_in_output"""
        # -- 原 test_no_gaps_ok --
        ...
        # -- 原 test_missing_dates_capped_at_20_in_output --
...
class TestDailyStockCoverage:
    ...
    def test_daily_stock_coverage_suite(self):
        """test_full_coverage_ok；test_low_coverage_warning；test_no_universe_skips_coverage"""
        # -- 原 test_full_coverage_ok --
        ...
        # -- 原 test_no_universe_skips_coverage --
...
class TestDailyBasicFieldNulls:
    ...
    def test_daily_basic_field_nulls_suite(self):
        """test_all_fields_present_ok；test_all_null_pb_warns；test_missing_column_warns"""
        # -- 原 test_all_fields_present_ok --
...
class TestFinancePITStaleness:
    def test_finance_pit_audit_suite(self):
        """test_fresh_ann_date_ok；test_stale_ann_date_warns；test_finance_unique_end_dates_reported；test_finance_field_null_rate_warning"""
        # -- 原 test_fresh_ann_date_ok --
        ...
        # -- 原 test_finance_unique_end_dates_reported --
        ...
        # -- 原 test_finance_field_null_rate_warning --
...
def test_storage_save_load_suite(tmp_dir):
    """test_save_and_load_roundtrip；test_save_append_deduplicates；test_save_append_replaces_existing_business_key；test_save_overwrite_replaces；test_hive_partitions_created；test_load_with_date_filter"""
    # -- 原 test_save_and_load_roundtrip --
    def _section_0_test_save_and_load_roundtrip(tmp_dir):
        df = _make_df(5)
    ...
    _td0 = tmp_dir / "_s0"
    _td0.mkdir(exist_ok=True)
    _section_0_test_save_and_load_roundtrip(_td0)
    # -- 原 test_save_append_deduplicates --
    def _section_1_test_save_append_deduplicates(tmp_dir):
        df1 = _make_df(5)
    ...
    _td1 = tmp_dir / "_s1"
    _td1.mkdir(exist_ok=True)
    _section_1_test_save_append_deduplicates(_td1)
    # -- 原 test_save_append_replaces_existing_business_key --
    def _section_2_test_save_append_replaces_existing_business_key(tmp_dir):
        original = _make_df(1)
    ...
    _td2 = tmp_dir / "_s2"
    _td2.mkdir(exist_ok=True)
    _section_2_test_save_append_replaces_existing_business_key(_td2)
    # -- 原 test_save_overwrite_replaces --
    def _section_3_test_save_overwrite_replaces(tmp_dir):
        df1 = _make_df(5)
    ...
    _td3 = tmp_dir / "_s3"
    _td3.mkdir(exist_ok=True)
    _section_3_test_save_overwrite_replaces(_td3)
    # -- 原 test_hive_partitions_created --
    def _section_4_test_hive_partitions_created(tmp_dir):
        df = _make_df(5)
        save_parquet(df, "test_data", base_dir=tmp_dir)
    ...
    _td4 = tmp_dir / "_s4"
    _td4.mkdir(exist_ok=True)
    _section_4_test_hive_partitions_created(_td4)
    # -- 原 test_load_with_date_filter --
    def _section_5_test_load_with_date_filter(tmp_dir):
        df = pl.DataFrame(
            {
    ...
    _td5 = tmp_dir / "_s5"
    _td5.mkdir(exist_ok=True)
    _section_5_test_load_with_date_filter(_td5)
def test_storage_end_boundary_suite(tmp_dir):
    """Datetime 列（分钟 bar）end 边界须含截止日全天，而非只到当日 00:00。；Date 列的 end 仍为闭区间（含截止日）。"""
    # -- 原 test_load_datetime_end_boundary_includes_full_end_day --
    def _section_0_test_load_datetime_end_boundary_includes_full_end_day(tmp_dir):
        from datetime import datetime
    ...
    _td0 = tmp_dir / "_s0"
    _td0.mkdir(exist_ok=True)
    _section_0_test_load_datetime_end_boundary_includes_full_end_day(_td0)
    # -- 原 test_load_date_end_boundary_still_inclusive --
    def _section_1_test_load_date_end_boundary_still_inclusive(tmp_dir):
        df = pl.DataFrame(
            {
    ...
    _td1 = tmp_dir / "_s1"
    _td1.mkdir(exist_ok=True)
    _section_1_test_load_date_end_boundary_still_inclusive(_td1)