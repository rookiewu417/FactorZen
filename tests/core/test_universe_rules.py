"""test_universe_board_limit.py：板块涨跌停阈值及 filter_limit 按板块细化测试。
test_universe_pit.py：S1-B 防回归：验证 get_universe("all_a") 实现 PIT 过滤（幸存者偏差消除）。
test_universe.py：测试指数成分股加载。
"""
import os
