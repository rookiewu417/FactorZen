"""
test_backtest.py：daily/evaluation/backtest.py 的单元测试。
test_backtest_direction.py：回测方向判定：IC 对齐（负 IC 自动取反）与 factor run / report 路径共用逻辑。
test_backtest_missing_weights_hold.py：快/慢路径对『signal 日在因子日历内但无预计算权重』的语义必须一致（D4）。
test_intraday_backtest.py：日内回测相关单元测试。
test_lookahead_safety.py：S0 防回归：验证策略回测不会使用同日收益。
"""
