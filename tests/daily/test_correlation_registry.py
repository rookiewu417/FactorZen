...
def test_factor_correlation_matrix_suite():
    """test_self_correlation_is_one；test_opposite_factor_correlation_is_negative；单因子应返回 1x1 矩阵，对角线为 1。；test_single_factor_returns_identity__factor_correlation_module；test_identical_factors_have_correlation_one"""
    # -- 原 test_self_correlation_is_one --
    def _section_0_test_self_correlation_is_one():
    ...
    _section_0_test_self_correlation_is_one()

    # -- 原 test_opposite_factor_correlation_is_negative --
    def _section_1_test_opposite_factor_correlation_is_negative():
        from factorzen.daily.evaluation.advanced import compute_factor_correlation
    ...
    _section_1_test_opposite_factor_correlation_is_negative()

    # -- 原 test_single_factor_returns_identity__factor_correlation --
    def _section_2_test_single_factor_returns_identity__factor_correlation():
        from factorzen.daily.evaluation.advanced import compute_factor_correlation
    ...
    _section_2_test_single_factor_returns_identity__factor_correlation()

    # -- 原 test_single_factor_returns_identity__factor_correlation_module --
    def _section_3_test_single_factor_returns_identity__factor_correlation_module():
        from factorzen.daily.evaluation.correlation import (
            CorrelationResult,
    ...
    _section_3_test_single_factor_returns_identity__factor_correlation_module()

    # -- 原 test_identical_factors_have_correlation_one --
    def _section_4_test_identical_factors_have_correlation_one():
    ...
    _section_4_test_identical_factors_have_correlation_one()
...
def test_factor_crowding_suite():
    """相关性矩阵对角线为 1.0。；pct_change(close, 20) 应与内置 momentum_20d 的 compute 输出一致。；vol==0（停牌）行价量被置 null → 因子值被过滤；vol>0 行正常产出。；compute() 必须先排序(ts_code, trade_date)再派生依赖行序的 ret_1d(shift().over())。"""
    # -- 原 test_crowding_diagonal_is_one --
    def _section_0_test_crowding_diagonal_is_one():
    ...
    _section_0_test_crowding_diagonal_is_one()

    # -- 原 test_expression_factor_matches_builtin_momentum --
    def _section_1_test_expression_factor_matches_builtin_momentum():
    ...
    _section_1_test_expression_factor_matches_builtin_momentum()

    # -- 原 test_suspended_rows_masked --
    def _section_2_test_suspended_rows_masked():
        from factorzen.discovery.factor import ExpressionFactor
    ...
    _section_2_test_suspended_rows_masked()

    # -- 原 test_ret_1d_correct_when_ctx_daily_rows_unsorted --
    def _section_3_test_ret_1d_correct_when_ctx_daily_rows_unsorted():
        from factorzen.discovery.factor import ExpressionFactor
    ...
    _section_3_test_ret_1d_correct_when_ctx_daily_rows_unsorted()
...
def test_factor_registry_discovery_suite():
    """test_plain_class_attrs_survive_instantiation；全量守卫：任何内置日频因子的类声明都不得在实例化时丢失。；test_finance_monthly_factors_declare_finance_and_daily；诚实标注：该因子是 YTD 累计口径，不应叫 roe_ttm 或在 description 声称 TTM。；test_generate_factor_proposal_parses_json；test_generate_factor_proposal_tolerates_garbage"""
    # -- 原 test_plain_class_attrs_survive_instantiation --
    def _section_0_test_plain_class_attrs_survive_instantiation():
    ...
    _section_0_test_plain_class_attrs_survive_instantiation()

    # -- 原 test_no_daily_factor_loses_its_declaration --
    def _section_1_test_no_daily_factor_loses_its_declaration():
    ...
    _section_1_test_no_daily_factor_loses_its_declaration()

    # -- 原 test_finance_monthly_factors_declare_finance_and_daily --
    def _section_2_test_finance_monthly_factors_declare_finance_and_daily():
    ...
    _section_2_test_finance_monthly_factors_declare_finance_and_daily()

    # -- 原 test_roe_factor_honestly_labeled_ytd_not_ttm --
    def _section_3_test_roe_factor_honestly_labeled_ytd_not_ttm():
        from factorzen.builtin_factors.monthly.profitability import RoeYtdMonthly

        assert RoeYtdMonthly.name == "roe_ytd"
        assert "TTM" not in RoeYtdMonthly.description or "非 TTM" in RoeYtdMonthly.description

    _section_3_test_roe_factor_honestly_labeled_ytd_not_ttm()

    # -- 原 test_generate_factor_proposal_parses_json --
    def _section_4_test_generate_factor_proposal_parses_json():
        raw = json.dumps(
            {
                "hypothesis": "低换手反转",
    ...
    _section_4_test_generate_factor_proposal_parses_json()

    # -- 原 test_generate_factor_proposal_tolerates_garbage --
    def _section_5_test_generate_factor_proposal_tolerates_garbage():
        llm = FakeLLM(["这不是 JSON"])
        props = generate_factor_proposal([{"role": "user", "content": "x"}], llm)
        assert props == []

    _section_5_test_generate_factor_proposal_tolerates_garbage()
