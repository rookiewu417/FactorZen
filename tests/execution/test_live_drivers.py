...
def test_replay_and_daily_step_suite(tmp_path):
    """容量约束须真的在 replay 路径生效：day1 提供成交额历史用于算 day2 的；test_replay_is_idempotent_on_rerun；test_daily_step_idempotent_and_resumes；test_daily_step_resume_nav_matches_single_shot_replay；扩窗/崩溃恢复：在已有部分 ledger 的 session 上再 run_replay（延长 --to），；run_replay 落的 state.json 须是可续跑态 {cash,pos,order_seq}，使后续；E1：signal_date=组合数据截止日(用当日收盘算权重)，须在**次一交易日**执行，"""
    # -- 原 test_replay_binds_adv_capacity_constraint --
    def _section_0_test_replay_binds_adv_capacity_constraint(tmp_path):
        code = "A.SZ"
    ...
    _section_0_test_replay_binds_adv_capacity_constraint(_tp0)

    # -- 原 test_replay_is_idempotent_on_rerun --
    def _section_1_test_replay_is_idempotent_on_rerun(tmp_path):
    ...
    _section_1_test_replay_is_idempotent_on_rerun(_tp1)

    # -- 原 test_daily_step_idempotent_and_resumes --
    def _section_2_test_daily_step_idempotent_and_resumes(tmp_path):
    ...
    _section_2_test_daily_step_idempotent_and_resumes(_tp2)

    # -- 原 test_daily_step_resume_nav_matches_single_shot_replay --
    def _section_3_test_daily_step_resume_nav_matches_single_shot_replay(tmp_path):
    ...
    _section_3_test_daily_step_resume_nav_matches_single_shot_replay(_tp3)

    # -- 原 test_replay_resume_extends_window_matches_single_shot --
    def _section_4_test_replay_resume_extends_window_matches_single_shot(tmp_path):
        d1, d2, d3 = date(2026, 1, 5), date(2026, 1, 6), date(2026, 1, 7)
    ...
    _section_4_test_replay_resume_extends_window_matches_single_shot(_tp4)

    # -- 原 test_replay_state_resumable_by_daily_step --
    def _section_5_test_replay_state_resumable_by_daily_step(tmp_path):
        d1, d2 = date(2026, 1, 5), date(2026, 1, 6)
    ...
    _section_5_test_replay_state_resumable_by_daily_step(_tp5)

    # -- 原 test_signal_executes_next_trading_day_not_same_day --
    def _section_6_test_signal_executes_next_trading_day_not_same_day(tmp_path):
    ...
    _section_6_test_signal_executes_next_trading_day_not_same_day(_tp6)
...
def test_live_cli_and_status_suite(tmp_path, capsys):
    """test_init_step_report_pipeline；test_live_cli_parser_routes_new_subcommands；test_live_status_handles_resumable_state_shape；test_live_status_handles_legacy_display_view_state"""
    # -- 原 test_init_step_report_pipeline --
    def _section_0_test_init_step_report_pipeline(tmp_path):
        dates = [date(2026, 1, 5), date(2026, 1, 6), date(2026, 1, 7)]
        daily = pl.DataFrame(
            [
                {
    ...
    _tp0 = tmp_path / "_s0"
    _tp0.mkdir(exist_ok=True)
    _section_0_test_init_step_report_pipeline(_tp0)

    # -- 原 test_live_cli_parser_routes_new_subcommands --
    def _section_1_test_live_cli_parser_routes_new_subcommands():
    ...
    _section_1_test_live_cli_parser_routes_new_subcommands()

    # -- 原 test_live_status_handles_resumable_state_shape --
    def _section_2_test_live_status_handles_resumable_state_shape(tmp_path, capsys):
    ...
    _tp2 = tmp_path / "_s2"
    _tp2.mkdir(exist_ok=True)
    _section_2_test_live_status_handles_resumable_state_shape(_tp2, capsys)

    # -- 原 test_live_status_handles_legacy_display_view_state --
    def _section_3_test_live_status_handles_legacy_display_view_state(tmp_path, capsys):
        from factorzen.cli.main import _cmd_live_status, build_parser
    ...
    _tp3 = tmp_path / "_s3"
    _tp3.mkdir(exist_ok=True)
    _section_3_test_live_status_handles_legacy_display_view_state(_tp3, capsys)
...
def test_live_sizing_and_st_suite(tmp_path):
    """执行日 pre_close 与 close 明显不同时，目标股数须按 pre_close 算（非 close）。；主板 ST 阈值 4.8%：开盘 +6% 对 ST 已涨停拒买；非 ST 未涨停应成交。；drivers 须构造 is_st_by_date 并写入 market entry，使 broker 对 ST 收窄阈值。"""
    # -- 原 test_daily_step_sizes_target_shares_with_pre_close_not_close --
    def _section_0_test_daily_step_sizes_target_shares_with_pre_close_not_close(tmp_path):
    ...
    _tp0 = tmp_path / "_s0"
    _tp0.mkdir(exist_ok=True)
    _section_0_test_daily_step_sizes_target_shares_with_pre_close_not_close(_tp0)

    # -- 原 test_paper_broker_st_limit_up_rejects_buy_when_is_st --
    def _section_1_test_paper_broker_st_limit_up_rejects_buy_when_is_st():
    ...
    _section_1_test_paper_broker_st_limit_up_rejects_buy_when_is_st()

    # -- 原 test_daily_step_passes_is_st_from_build_is_st_by_date --
    def _section_2_test_daily_step_passes_is_st_from_build_is_st_by_date(tmp_path, mp):
        sig, exec_d = date(2026, 1, 5), date(2026, 1, 6)
        code = "600001.SH"
    ...
    with pytest.MonkeyPatch.context() as mp:
        _section_2_test_daily_step_passes_is_st_from_build_is_st_by_date(_tp2, mp)
