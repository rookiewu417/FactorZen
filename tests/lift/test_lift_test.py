...
def test_run_lift_tests_engine_suite():
    """正交强信号候选 lift 过阈值；纯噪声候选 lift 更低/拒。residual_ic_v1 口径。；top_m 截断不静默：返回行带 n_input / n_selected。；test_lift_bad_candidate_does_not_crash_batch；错误路径 row 也有 admission_ic 键（形态一致，下游 no KeyError）。"""
    # -- 原 test_lift_signal_candidate_passes_noise_fails --
    def _section_0_test_lift_signal_candidate_passes_noise_fails():
    ...
    _section_0_test_lift_signal_candidate_passes_noise_fails()

    # -- 原 test_lift_top_m_truncation_recorded --
    def _section_1_test_lift_top_m_truncation_recorded():
    ...
    _section_1_test_lift_top_m_truncation_recorded()

    # -- 原 test_lift_bad_candidate_does_not_crash_batch --
    def _section_2_test_lift_bad_candidate_does_not_crash_batch():
        from factorzen.discovery.lift_test import run_lift_tests
    ...
    _section_2_test_lift_bad_candidate_does_not_crash_batch()

    # -- 原 test_run_lift_tests_error_rows_have_admission_ic_key --
    def _section_3_test_run_lift_tests_error_rows_have_admission_ic_key():
        from factorzen.discovery.lift_test import run_lift_tests
    ...
    _section_3_test_run_lift_tests_error_rows_have_admission_ic_key()
...
def test_paired_lift_stats_suite():
    """手算：lift / SE / 半段 / 奇数块 / 尾块不足 / n_blocks=1 → SE=None。；日期完全相同时配对 lift == 两序列均值之差。；日期不齐时只在交集上配对（比各自均值之差更对）。"""
    # -- 原 test_paired_lift_stats_hand_calc --
    def _section_0_test_paired_lift_stats_hand_calc():
    ...
    _section_0_test_paired_lift_stats_hand_calc()

    # -- 原 test_paired_lift_equals_mean_diff_when_dates_align --
    def _section_1_test_paired_lift_equals_mean_diff_when_dates_align():
        from factorzen.discovery.lift_test import paired_lift_stats
    ...
    _section_1_test_paired_lift_equals_mean_diff_when_dates_align()

    # -- 原 test_paired_lift_uses_inner_join_dates --
    def _section_2_test_paired_lift_uses_inner_join_dates():
        from factorzen.discovery.lift_test import paired_lift_stats
    ...
    _section_2_test_paired_lift_uses_inner_join_dates()
...
def test_run_lift_fields_suite():
    """admission_ic = 单因子 admission 窗 RankIC，正/负相关各得对应符号；非残差 IC。；新字段全链 + top_m=None 全测；正交信号 → residual lift 过阈值。；production 形态 run_lift_tests：row 含 admission/scored/block/baseline_hash。"""
    # -- 原 test_run_lift_tests_admission_ic_reflects_single_factor_sign --
    def _section_0_test_run_lift_tests_admission_ic_reflects_single_factor_sign():
        from factorzen.discovery.lift_test import run_lift_tests
    ...
    _section_0_test_run_lift_tests_admission_ic_reflects_single_factor_sign()

    # -- 原 test_run_lift_tests_new_fields_and_top_m_none --
    def _section_1_test_run_lift_tests_new_fields_and_top_m_none():
        from factorzen.discovery.lift_test import run_lift_tests
    ...
    _section_1_test_run_lift_tests_new_fields_and_top_m_none()

    # -- 原 test_run_lift_tests_admission_provenance_complete --
    def _section_2_test_run_lift_tests_admission_provenance_complete():
        import hashlib
    ...
    _section_2_test_run_lift_tests_admission_provenance_complete()
...
def test_upsert_probation_suite(tmp_path):
    """test_upsert_probation_roundtrip_fields；test_upsert_probation_skips_not_passed；|IC| 远低于 floor 仍可 probation——不走单因子 gate。"""
    # -- 原 test_upsert_probation_roundtrip_fields --
    def _section_0_test_upsert_probation_roundtrip_fields(tmp_path):
    ...
    _section_0_test_upsert_probation_roundtrip_fields(_tp0)

    # -- 原 test_upsert_probation_skips_not_passed --
    def _section_1_test_upsert_probation_skips_not_passed(tmp_path):
        from factorzen.discovery.factor_library import load_library, upsert_probation
    ...
    _section_1_test_upsert_probation_skips_not_passed(_tp1)

    # -- 原 test_upsert_probation_skips_library_gate --
    def _section_2_test_upsert_probation_skips_library_gate():
        import tempfile
    ...
    _section_2_test_upsert_probation_skips_library_gate()


def test_probation_lifecycle_suite(tmp_path):
    """test_rebuild_preserves_probation；test_build_library_pool_excludes_probation_by_default"""
    # -- 原 test_rebuild_preserves_probation --
    def _section_0_test_rebuild_preserves_probation(tmp_path):
    ...
    _tp0 = tmp_path / "_s0"
    _tp0.mkdir(exist_ok=True)
    _section_0_test_rebuild_preserves_probation(_tp0)

    # -- 原 test_build_library_pool_excludes_probation_by_default --
    def _section_1_test_build_library_pool_excludes_probation_by_default(tmp_path):
    ...
    _tp1 = tmp_path / "_s1"
    _tp1.mkdir(exist_ok=True)
    _section_1_test_build_library_pool_excludes_probation_by_default(_tp1)
