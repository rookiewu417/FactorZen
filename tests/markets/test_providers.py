...
# ── 测试 ─────────────────────────────────────────────────────────────────────
def test_ports_and_profile_suite():
    """7 个 Port 均为抽象类，不可直接实例化。；MarketProfile 打包 7 个 port + 元数据，且 frozen。；RiskModel 可为 None（crypto 本期延后到 MC3 填）。；registry：register→get 返回同一 profile（缓存），list 含其名。；未注册市场 get 抛 KeyError。"""
    # -- 原 test_ports_are_abstract --
    def _section_0_test_ports_are_abstract():
        for cls in [DataProvider, Calendar, RiskModel]:
            with pytest.raises(TypeError):
                cls()  # type: ignore[abstract]

    _section_0_test_ports_are_abstract()

    # -- 原 test_market_profile_bundles_ports --
    def _section_1_test_market_profile_bundles_ports():
        p = _dummy_profile()
        assert p.name == "dummy"
    ...
    _section_1_test_market_profile_bundles_ports()

    # -- 原 test_risk_is_optional --
    def _section_2_test_risk_is_optional():
        p = _dummy_profile()
        p2 = MarketProfile(
            name="norisk",
            quote_currency="XXX",
    ...
    _section_2_test_risk_is_optional()

    # -- 原 test_registry_register_get_list --
    def _section_3_test_registry_register_get_list():
    ...
    _section_3_test_registry_register_get_list()

    # -- 原 test_registry_unknown_raises --
    def _section_4_test_registry_unknown_raises():
        with pytest.raises(KeyError):
            registry.get("__nonexistent_market__")

    _section_4_test_registry_unknown_raises()
...
def test_ashare_adjustment_suite():
    """test_split_adjustment_ground_truth；test_no_corporate_action_adjusted_equals_raw；test_null_rows_dropped"""
    # -- 原 test_split_adjustment_ground_truth --
    def _section_0_test_split_adjustment_ground_truth():
    ...
    _section_0_test_split_adjustment_ground_truth()

    # -- 原 test_no_corporate_action_adjusted_equals_raw --
    def _section_1_test_no_corporate_action_adjusted_equals_raw():
    ...
    _section_1_test_no_corporate_action_adjusted_equals_raw()

    # -- 原 test_null_rows_dropped --
    def _section_2_test_null_rows_dropped():
    ...
    _section_2_test_null_rows_dropped()
...
def test_ashare_provider_fetch_suite(tmp_path):
    """test_fetch_bars_and_cache_audit；test_fetch_bars_filters_window；test_fetch_symbol_meta_from_snapshot"""
    # -- 原 test_fetch_bars_and_cache_audit --
    def _section_0_test_fetch_bars_and_cache_audit(tmp_path):
        counter: dict = {}
    ...
    _section_0_test_fetch_bars_and_cache_audit(_tp0)

    # -- 原 test_fetch_bars_filters_window --
    def _section_1_test_fetch_bars_filters_window(tmp_path):
        counter: dict = {}
        prov = USDataProvider(cache_root=str(tmp_path), fetch=_fake_fetch_factory(counter),
                              request_interval=0.0)
    ...
    _section_1_test_fetch_bars_filters_window(_tp1)

    # -- 原 test_fetch_symbol_meta_from_snapshot --
    def _section_2_test_fetch_symbol_meta_from_snapshot(tmp_path):
    ...
    _section_2_test_fetch_symbol_meta_from_snapshot(_tp2)
...
def test_ccxt_provider_suite():
    """test_is_a_dataprovider；test_symbol_mapping_roundtrip；test_fetch_bars_schema_and_values；test_fetch_bars_respects_end_date；日频 funding = 当日多档 funding 之和（Binance 每 8h 一档）。；test_fetch_open_interest；test_fetch_symbol_meta_only_swap_quote；test_fetch_funding_paginates_beyond_1000；test_fetch_open_interest_daily_timeframe_and_aggregation"""
    # -- 原 test_is_a_dataprovider --
    def _section_0_test_is_a_dataprovider():
        assert isinstance(_provider(), DataProvider)

    _section_0_test_is_a_dataprovider()

    # -- 原 test_symbol_mapping_roundtrip --
    def _section_1_test_symbol_mapping_roundtrip():
        p = _provider()
        assert p._to_unified("BTCUSDT") == "BTC/USDT:USDT"
        assert p._to_ts_code("BTC/USDT:USDT") == "BTCUSDT"

    _section_1_test_symbol_mapping_roundtrip()

    # -- 原 test_fetch_bars_schema_and_values --
    def _section_2_test_fetch_bars_schema_and_values():
        p = _provider()
    ...
    _section_2_test_fetch_bars_schema_and_values()

    # -- 原 test_fetch_bars_respects_end_date --
    def _section_3_test_fetch_bars_respects_end_date():
        p = _provider()
        df = p.fetch_bars(["BTCUSDT"], "20240101", "20240102")  # 只到 1/2
        assert df.height == 2

    _section_3_test_fetch_bars_respects_end_date()

    # -- 原 test_fetch_funding_daily_sum --
    def _section_4_test_fetch_funding_daily_sum():
        p = _provider()
    ...
    _section_4_test_fetch_funding_daily_sum()

    # -- 原 test_fetch_open_interest --
    def _section_5_test_fetch_open_interest():
        p = _provider()
        oi = p.fetch_open_interest(["BTCUSDT"], "20240101", "20240103")
        assert set(oi.columns) >= {"ts_code", "trade_date", "open_interest"}
        assert oi.height == 2

    _section_5_test_fetch_open_interest()

    # -- 原 test_fetch_symbol_meta_only_swap_quote --
    def _section_6_test_fetch_symbol_meta_only_swap_quote():
        p = _provider()
    ...
    _section_6_test_fetch_symbol_meta_only_swap_quote()

    # -- 原 test_fetch_funding_paginates_beyond_1000 --
    def _section_7_test_fetch_funding_paginates_beyond_1000():
    ...
    _section_7_test_fetch_funding_paginates_beyond_1000()

    # -- 原 test_fetch_open_interest_daily_timeframe_and_aggregation --
    def _section_8_test_fetch_open_interest_daily_timeframe_and_aggregation():
    ...
    _section_8_test_fetch_open_interest_daily_timeframe_and_aggregation()
...
def test_futures_roll_adjustment_suite():
    """两合约一次展期，逐值对手工计算的后复权 close。；三合约两次展期，累乘 adj_factor 逐值校验（防单展期恰好巧合）。；次主力连续（L 后缀）经 fut_codes 过滤，只留主力连续。；新主力在展期前一日无报价 → 该展期不复权（roll_step=1），诚实退化不崩。；test_empty_inputs"""
    # -- 原 test_single_roll_ground_truth --
    def _section_0_test_single_roll_ground_truth():
    ...
    _section_0_test_single_roll_ground_truth()

    # -- 原 test_two_rolls_cumulative_ground_truth --
    def _section_1_test_two_rolls_cumulative_ground_truth():
    ...
    _section_1_test_two_rolls_cumulative_ground_truth()

    # -- 原 test_secondary_L_code_filtered --
    def _section_2_test_secondary_L_code_filtered():
    ...
    _section_2_test_secondary_L_code_filtered()

    # -- 原 test_missing_new_prev_close_no_adjust --
    def _section_3_test_missing_new_prev_close_no_adjust():
    ...
    _section_3_test_missing_new_prev_close_no_adjust()

    # -- 原 test_empty_inputs --
    def _section_4_test_empty_inputs():
    ...
    _section_4_test_empty_inputs()


# ==== 来自 test_ashare_wrap.py ====
def test_ashare_ports_impl_suite():
    """test_calendar_periods_per_year_252；叶子字典与 discovery.operators 同源（避免漂移）。；A 股派生：vwap=amount/vol, log_vol=ln(vol+1), ret_1d 用 close_adj。；卖出含印花税、买入不含（A 股关键差异）。；long-only：多头无持有成本；空头计融券利息。；test_rules_long_only_t1；AShareDataProvider 仅经 fetch_daily 取日频；非 daily freq 须显式报错，；freq='daily'（默认）委托 core.loader.fetch_daily，参数透传。；test_registry_get_ashare"""
    # -- 原 test_calendar_periods_per_year_252 --
    def _section_0_test_calendar_periods_per_year_252():
        cal = AShareCalendar()
    ...
    _section_0_test_calendar_periods_per_year_252()

    # -- 原 test_factorset_leaves_match_operators --
    def _section_1_test_factorset_leaves_match_operators():
        fs = AShareFactorSet()
        assert fs.leaf_features() == LEAF_FEATURES
        assert fs.basic_features() == BASIC_FEATURES

    _section_1_test_factorset_leaves_match_operators()

    # -- 原 test_factorset_derived_columns_ashare_convention --
    def _section_2_test_factorset_derived_columns_ashare_convention():
        fs = AShareFactorSet()
        bars = pl.DataFrame({
            "ts_code": ["000001.SZ"] * 3,
    ...
    _section_2_test_factorset_derived_columns_ashare_convention()

    # -- 原 test_cost_stamp_tax_asymmetry --
    def _section_3_test_cost_stamp_tax_asymmetry():
        c = AShareCostModel()
    ...
    _section_3_test_cost_stamp_tax_asymmetry()

    # -- 原 test_cost_carry_long_only --
    def _section_4_test_cost_carry_long_only():
        c = AShareCostModel()
        assert c.carry_cost(10000.0, 5) == 0.0  # 多头无成本
        assert c.carry_cost(-10000.0, 5) > 0.0  # 空头有融券利息

    _section_4_test_cost_carry_long_only()

    # -- 原 test_rules_long_only_t1 --
    def _section_5_test_rules_long_only_t1():
    ...
    _section_5_test_rules_long_only_t1()

    # -- 原 test_ashare_provider_fetch_bars_rejects_non_daily --
    def _section_6_test_ashare_provider_fetch_bars_rejects_non_daily():
    ...
    _section_6_test_ashare_provider_fetch_bars_rejects_non_daily()

    # -- 原 test_ashare_provider_fetch_bars_daily_delegates --
    def _section_7_test_ashare_provider_fetch_bars_daily_delegates(mp):
        ...
        mp.setattr(loader, "fetch_daily", _fake_fetch_daily)
    ...
    with pytest.MonkeyPatch.context() as mp:
        _section_7_test_ashare_provider_fetch_bars_daily_delegates(mp)

    # -- 原 test_registry_get_ashare --
    def _section_8_test_registry_get_ashare():
    ...
    _section_8_test_registry_get_ashare()
