...
def test_constraints_suite():
    """test_box_and_budget；test_industry_neutral_to_zero；test_turnover_constraint；test_infeasible_when_w_max_too_tight；复现 Critical 根因：生产形态 raw one-hot 行业列 + neutral=所有 ind_ + bench=None；同 raw one-hot 行业列，传等权基准 → target = 等权行业暴露（各行业 1/n_ind），"""
    # -- 原 test_box_and_budget --
    def _section_0_test_box_and_budget():
    ...
    _section_0_test_box_and_budget()

    # -- 原 test_industry_neutral_to_zero --
    def _section_1_test_industry_neutral_to_zero():
    ...
    _section_1_test_industry_neutral_to_zero()

    # -- 原 test_turnover_constraint --
    def _section_2_test_turnover_constraint():
    ...
    _section_2_test_turnover_constraint()

    # -- 原 test_infeasible_when_w_max_too_tight --
    def _section_3_test_infeasible_when_w_max_too_tight():
        n = 6
    ...
    _section_3_test_infeasible_when_w_max_too_tight()

    # -- 原 test_raw_onehot_industry_neutral_no_bench_is_infeasible --
    def _section_4_test_raw_onehot_industry_neutral_no_bench_is_infeasible():
    ...
    _section_4_test_raw_onehot_industry_neutral_no_bench_is_infeasible()

    # -- 原 test_raw_onehot_industry_neutral_with_equal_bench_is_feasible --
    def _section_5_test_raw_onehot_industry_neutral_with_equal_bench_is_feasible():
        exp, ind_names = _raw_onehot_exposures()
        n = exp.n_stocks
    ...
    _section_5_test_raw_onehot_industry_neutral_with_equal_bench_is_feasible()
...
def test_optimize_portfolio_suite():
    """test_optimize_returns_optimal_weights；矛盾约束(w_max 太小无法满仓) → infeasible，weights=None，不返回垃圾。；CLARABEL 因数值精度返回 'optimal_inaccurate'（AlmostSolved）时 w.value 非 None，；非PSD协方差矩阵在 prob.solve() 阶段抛 cp.error.DCPError(而非 SolverError)。"""
    # -- 原 test_optimize_returns_optimal_weights --
    def _section_0_test_optimize_returns_optimal_weights():
    ...
    _section_0_test_optimize_returns_optimal_weights()

    # -- 原 test_infeasible_returns_none_not_garbage --
    def _section_1_test_infeasible_returns_none_not_garbage():
    ...
    _section_1_test_infeasible_returns_none_not_garbage()

    # -- 原 test_optimize_accepts_optimal_inaccurate --
    def _section_2_test_optimize_accepts_optimal_inaccurate(mp):
    ...
    with pytest.MonkeyPatch.context() as mp:
        _section_2_test_optimize_accepts_optimal_inaccurate(mp)

    # -- 原 test_non_psd_covariance_dcp_error_returns_error_status_not_raises --
    def _section_3_test_non_psd_covariance_dcp_error_returns_error_status_not_raises(mp):
        mp.setattr(optimizer_mod, "_psd", lambda F: F)
    ...
    with pytest.MonkeyPatch.context() as mp:
        _section_3_test_non_psd_covariance_dcp_error_returns_error_status_not_raises(mp)
...
def test_run_portfolio_pipeline_suite(tmp_path, capsys):
    """test_run_portfolio_writes_products；建仓时 stock_returns=zeros + factor_returns_latest={} → manifest 含占位标注。；传入真实收益时 return_attribution_available=True，note=None。；run_portfolio 传 signal_date 后，manifest.json 应记录该字段供 M7 sim 对齐。；w_max=0.1 → n*w_max=0.6 < 1，违反 Σw=1，优化器返回 infeasible，pipeline 不崩溃。；manifest.json 应含 command/git_dirty/pixi_lock_sha256/schema_version（复用 core.exper；显式传 command 时应原样记录，供复现当时具体怎么跑的。；L2：给了 turnover_budget 但无 prev_weights → 换手约束静默丢弃，须告警。"""
    # -- 原 test_run_portfolio_writes_products --
    def _section_0_test_run_portfolio_writes_products(tmp_path):
    ...
    _section_0_test_run_portfolio_writes_products(_tp0)

    # -- 原 test_run_portfolio_attribution_placeholder_flag --
    def _section_1_test_run_portfolio_attribution_placeholder_flag(tmp_path):
        rr = _risk_result__pipeline()
        alpha = np.array([0.1, 0.05, 0.02, 0.08, 0.03, 0.01])
        res = run_portfolio(alpha, rr, codes=rr.factor_exposures.codes,
    ...
    _section_1_test_run_portfolio_attribution_placeholder_flag(_tp1)

    # -- 原 test_run_portfolio_attribution_available_when_returns_provided --
    def _section_2_test_run_portfolio_attribution_available_when_returns_provided(tmp_path):
        rr = _risk_result__pipeline()
        alpha = np.array([0.1, 0.05, 0.02, 0.08, 0.03, 0.01])
        res = run_portfolio(alpha, rr, codes=rr.factor_exposures.codes,
                            stock_returns=np.array([0.03, 0.01, -0.02, 0.04, 0.0, 0.01]),
    ...
    _section_2_test_run_portfolio_attribution_available_when_returns_provided(_tp2)

    # -- 原 test_run_portfolio_records_signal_date --
    def _section_3_test_run_portfolio_records_signal_date(tmp_path):
        rr = _risk_result__pipeline()
        alpha = np.array([0.1, 0.05, 0.02, 0.08, 0.03, 0.01])
        res = run_portfolio(alpha, rr, codes=rr.factor_exposures.codes,
    ...
    _section_3_test_run_portfolio_records_signal_date(_tp3)

    # -- 原 test_run_portfolio_infeasible_does_not_crash --
    def _section_4_test_run_portfolio_infeasible_does_not_crash(tmp_path):
    ...
    _section_4_test_run_portfolio_infeasible_does_not_crash(_tp4)

    # -- 原 test_run_portfolio_manifest_has_reproducibility_fields --
    def _section_5_test_run_portfolio_manifest_has_reproducibility_fields(tmp_path):
        rr = _risk_result__pipeline()
        alpha = np.array([0.1, 0.05, 0.02, 0.08, 0.03, 0.01])
        res = run_portfolio(alpha, rr, codes=rr.factor_exposures.codes,
                            stock_returns=np.array([0.03, 0.01, -0.02, 0.04, 0.0, 0.01]),
    ...
    _section_5_test_run_portfolio_manifest_has_reproducibility_fields(_tp5)

    # -- 原 test_run_portfolio_manifest_command_override --
    def _section_6_test_run_portfolio_manifest_command_override(tmp_path):
        rr = _risk_result__pipeline()
        alpha = np.array([0.1, 0.05, 0.02, 0.08, 0.03, 0.01])
        res = run_portfolio(alpha, rr, codes=rr.factor_exposures.codes,
                            stock_returns=np.array([0.03, 0.01, -0.02, 0.04, 0.0, 0.01]),
    ...
    _section_6_test_run_portfolio_manifest_command_override(_tp6)

    # -- 原 test_run_portfolio_warns_when_turnover_set_without_prev_weights --
    def _section_7_test_run_portfolio_warns_when_turnover_set_without_prev_weights(tmp_path, capsys):
        rr = _risk_result__pipeline()
        alpha = np.array([0.1, 0.05, 0.02, 0.08, 0.03, 0.01])
    ...
    _section_7_test_run_portfolio_warns_when_turnover_set_without_prev_weights(_tp7, capsys)
...
def test_portfolio_report_html_suite(base_metrics, attribution_df, risk_df, manifest):
    """复刻 brief 里要求的断言，不依赖真实回测。；输出是合法 HTML 文档，以 DOCTYPE 开头。；sim_result=None 时不应有 base64 图表字符串（无 <img src='data:image）。；修复5：能力总览卡按能力命名展示，不暴露内部里程碑代号（M1-M6）。"""
    # -- 原 test_generate_portfolio_report_html_has_sections --
    def _section_0_test_generate_portfolio_report_html_has_sections(base_metrics, attribution_df, risk_df, manifest):
    ...
    _section_0_test_generate_portfolio_report_html_has_sections(base_metrics, attribution_df, risk_df, manifest)

    # -- 原 test_html_has_doctype --
    def _section_1_test_html_has_doctype(base_metrics):
        html = generate_portfolio_report(None, metrics=base_metrics)
        assert html.strip().startswith("<!DOCTYPE") or html.strip().startswith("<!")

    _section_1_test_html_has_doctype(base_metrics)

    # -- 原 test_no_chart_when_sim_result_none --
    def _section_2_test_no_chart_when_sim_result_none(base_metrics):
        html = generate_portfolio_report(None, metrics=base_metrics)
        # If there are no charts, there should be no base64 img tags
        assert 'data:image/png;base64' not in html

    _section_2_test_no_chart_when_sim_result_none(base_metrics)

    # -- 原 test_capability_overview_cards_present --
    def _section_3_test_capability_overview_cards_present(base_metrics):
        html = generate_portfolio_report(None, metrics=base_metrics)
        # 按能力命名的文案应存在
        assert any(
            token in html
    ...
    _section_3_test_capability_overview_cards_present(base_metrics)
...
def test_portfolio_build_sim_integration_suite(tmp_path):
    """run_portfolio() 真实落盘的 run_dir 直接喂给 run_portfolio_simulation()：；一个 optimal + 一个 infeasible（w_max 过小导致约束不可行）的 run_dir 一起喂给 sim：；所有 run_dir 都是 infeasible 兜底时，sim 应彻底找不到有效信号并明确报错，"""
    # -- 原 test_portfolio_build_to_sim_happy_path --
    def _section_0_test_portfolio_build_to_sim_happy_path(tmp_path):
    ...
    _tp0 = tmp_path / "_s0"
    _tp0.mkdir(exist_ok=True)
    _section_0_test_portfolio_build_to_sim_happy_path(_tp0)

    # -- 原 test_portfolio_build_infeasible_status_not_treated_as_valid_signal --
    def _section_1_test_portfolio_build_infeasible_status_not_treated_as_valid_signal(tmp_path):
    ...
    _tp1 = tmp_path / "_s1"
    _tp1.mkdir(exist_ok=True)
    _section_1_test_portfolio_build_infeasible_status_not_treated_as_valid_signal(_tp1)

    # -- 原 test_portfolio_build_only_infeasible_run_raises --
    def _section_2_test_portfolio_build_only_infeasible_run_raises(tmp_path):
        rr = _risk_result__integration()
        codes = rr.factor_exposures.codes
    ...
    _tp2 = tmp_path / "_s2"
    _tp2.mkdir(exist_ok=True)
    _section_2_test_portfolio_build_only_infeasible_run_raises(_tp2)


# ==== 来自 test_attribution.py ====
# ==== 来自 test_attribution_brinson.py ====
def test_brinson_and_risk_attr_suite():
    """配置 + 选股 ≈ 组合超额收益。；组合行业内选股与基准一致、仅行业权重不同 → 全是配置效应。；sectors 含 None 时不崩溃；None 归入 "" 行业；守恒仍成立。；因子收益贡献 + 特异 ≈ 组合收益(因子模型口径)。；风险贡献与 M3 decompose_risk 一致(跨函数验证,非恒真)。"""
    # -- 原 test_brinson_conserves_to_excess --
    def _section_0_test_brinson_conserves_to_excess():
    ...
    _section_0_test_brinson_conserves_to_excess()

    # -- 原 test_pure_allocation --
    def _section_1_test_pure_allocation():
    ...
    _section_1_test_pure_allocation()

    # -- 原 test_brinson_handles_none_sectors --
    def _section_2_test_brinson_handles_none_sectors():
    ...
    _section_2_test_brinson_handles_none_sectors()

    # -- 原 test_return_attribution_conserves --
    def _section_3_test_return_attribution_conserves():
    ...
    _section_3_test_return_attribution_conserves()

    # -- 原 test_risk_contrib_matches_m3_decompose --
    def _section_4_test_risk_contrib_matches_m3_decompose():
    ...
    _section_4_test_risk_contrib_matches_m3_decompose()
