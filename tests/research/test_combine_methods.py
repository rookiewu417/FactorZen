...
def test_combine_from_session_suite(tmp_path):
    """因子库(≥2 因子)→ 物化 + 收益面板 + 四方法 OOS 对比,返回 comparison。；因子库不足 2 个 → 明确报错(组合至少需两个,不静默产垃圾)。；默认只取 passed=True 的库因子;过滤后不足 2 个则报错。；两个 session 各含同一表达式（规范形相同）→ 合并后只出现一次。；构造高相关对（ts_mean(close,20) 与 ts_mean(close,21)）→ 仅 |holdout_ic| 高者存活，；decorr_threshold=1.0 → 逃生口，无剔除。；去相关后 < 2 个因子 → 报错（组合至少需两个）。"""
    # -- 原 test_combine_from_session_end_to_end --
    def _section_0_test_combine_from_session_end_to_end(tmp_path, mp):
    ...
    with pytest.MonkeyPatch.context() as mp:
        _section_0_test_combine_from_session_end_to_end(_tp0, mp)

    # -- 原 test_combine_from_session_needs_two_factors --
    def _section_1_test_combine_from_session_needs_two_factors(tmp_path, mp):
        mp.setattr("factorzen.pipelines.factor_mine.prepare_mining_daily", lambda *a, **k: _daily())
    ...
    with pytest.MonkeyPatch.context() as mp:
        _section_1_test_combine_from_session_needs_two_factors(_tp1, mp)

    # -- 原 test_combine_from_session_passed_only_filters --
    def _section_2_test_combine_from_session_passed_only_filters(tmp_path, mp):
        mp.setattr("factorzen.pipelines.factor_mine.prepare_mining_daily", lambda *a, **k: _daily())
        sess = tmp_path / "s2"
        sess.mkdir()
    ...
    with pytest.MonkeyPatch.context() as mp:
        _section_2_test_combine_from_session_passed_only_filters(_tp2, mp)

    # -- 原 test_combine_merges_and_dedups_across_sessions --
    def _section_3_test_combine_merges_and_dedups_across_sessions(tmp_path, mp):
        mp.setattr("factorzen.pipelines.factor_mine.prepare_mining_daily", lambda *a, **k: _daily())
    ...
    with pytest.MonkeyPatch.context() as mp:
        _section_3_test_combine_merges_and_dedups_across_sessions(_tp3, mp)

    # -- 原 test_combine_decorr_drops_near_duplicate --
    def _section_4_test_combine_decorr_drops_near_duplicate(tmp_path, mp):
        mp.setattr("factorzen.pipelines.factor_mine.prepare_mining_daily", lambda *a, **k: _daily())
    ...
    with pytest.MonkeyPatch.context() as mp:
        _section_4_test_combine_decorr_drops_near_duplicate(_tp4, mp)

    # -- 原 test_combine_decorr_threshold_one_keeps_all --
    def _section_5_test_combine_decorr_threshold_one_keeps_all(tmp_path, mp):
        mp.setattr("factorzen.pipelines.factor_mine.prepare_mining_daily", lambda *a, **k: _daily())
    ...
    with pytest.MonkeyPatch.context() as mp:
        _section_5_test_combine_decorr_threshold_one_keeps_all(_tp5, mp)

    # -- 原 test_combine_decorr_below_two_errors --
    def _section_6_test_combine_decorr_below_two_errors(tmp_path, mp):
        mp.setattr("factorzen.pipelines.factor_mine.prepare_mining_daily", lambda *a, **k: _daily())
        sess = _session_with_ic(tmp_path, "s", [("ts_mean(close,20)", 0.08), ("ts_mean(close,21)", 0.03)])
        with pytest.raises(ValueError, match="不足 2 个"):
            factor_combine.combine_from_session(
    ...
    with pytest.MonkeyPatch.context() as mp:
        _section_6_test_combine_decorr_below_two_errors(_tp6, mp)
...
def test_net_metrics_turnover_suite():
    """持仓完全不变 ⇒ 换手 0 ⇒ 净 spread 必须**逐位等于**毛 spread。；每期 top 桶**整桶换掉** ⇒ 换手 1.0 ⇒ 净 = 毛 − 4×1.0×10bp。；换手 0.5（2 只里换 1 只）⇒ 费用恰为全换的一半。；逐期序列的均值必须等于聚合版——两者是同一口径的两种取法。；空面板也须带上新键，否则消费方 `r['net_spread_10bp']` 会 KeyError。；净收益方差为 0（恒定）⇒ SR 定义为 0，不得抛除零或返 inf/nan。"""
    # -- 原 test_zero_turnover_net_equals_gross --
    def _section_0_test_zero_turnover_net_equals_gross():
    ...
    _section_0_test_zero_turnover_net_equals_gross()

    # -- 原 test_full_turnover_charges_expected_fee --
    def _section_1_test_full_turnover_charges_expected_fee():
    ...
    _section_1_test_full_turnover_charges_expected_fee()

    # -- 原 test_half_turnover_is_between --
    def _section_2_test_half_turnover_is_between():
    ...
    _section_2_test_half_turnover_is_between()

    # -- 原 test_turnover_series_matches_aggregate --
    def _section_3_test_turnover_series_matches_aggregate():
        import numpy as np
    ...
    _section_3_test_turnover_series_matches_aggregate()

    # -- 原 test_empty_panel_has_net_keys --
    def _section_4_test_empty_panel_has_net_keys():
        empty = pl.DataFrame(
    ...
    _section_4_test_empty_panel_has_net_keys()

    # -- 原 test_net_sharpe_zero_when_no_variance --
    def _section_5_test_net_sharpe_zero_when_no_variance():
        days = [_stable_day(i, ["S0", "S1"]) for i in range(6)]
        combined, ret_df = _panel(days)
        out = _evaluate_oos(combined, ret_df, n_groups=N_GROUPS)
        assert out["net_sharpe_10bp"] == 0.0

    _section_5_test_net_sharpe_zero_when_no_variance()
...
def test_greedy_decorrelate_suite():
    """随机 3 组合成面板：新旧 kept 序 + dropped(expression/corr_with) 完全一致，corr≤1e-9。；threshold=1.0 → >1.0 恒 False → 全 kept、dropped 空。；一因子只有半段日期、另一因子缺部分股票：不崩且与旧实现一致。；~20 因子 × 250 日 × 100 股：打印 A/B 耗时；新实现预期 ≪ 旧（不硬断言防抖动）。"""
    # -- 原 test_greedy_decorrelate_decision_parity_three_seeds --
    def _section_0_test_greedy_decorrelate_decision_parity_three_seeds():
    ...
    _section_0_test_greedy_decorrelate_decision_parity_three_seeds()

    # -- 原 test_greedy_decorrelate_threshold_one_keeps_all --
    def _section_1_test_greedy_decorrelate_threshold_one_keeps_all():
        from factorzen.pipelines.factor_combine import (
            _greedy_decorrelate,
            _greedy_decorrelate_reference,
        )
    ...
    _section_1_test_greedy_decorrelate_threshold_one_keeps_all()

    # -- 原 test_greedy_decorrelate_heterogeneous_coverage_parity --
    def _section_2_test_greedy_decorrelate_heterogeneous_coverage_parity():
        from factorzen.pipelines.factor_combine import (
            _greedy_decorrelate,
            _greedy_decorrelate_reference,
        )
    ...
    _section_2_test_greedy_decorrelate_heterogeneous_coverage_parity()

    # -- 原 test_greedy_decorrelate_scaled_perf_smoke --
    def _section_3_test_greedy_decorrelate_scaled_perf_smoke():
        from factorzen.pipelines.factor_combine import (
            _greedy_decorrelate,
            _greedy_decorrelate_reference,
        )
    ...
    _section_3_test_greedy_decorrelate_scaled_perf_smoke()
...
def test_factor_instantiate_and_return_frame_suite():
    """test_instantiate_factor_builds_instance_from_registry_class；test_prepare_return_frame_adds_ret_and_forward_returns"""
    # -- 原 test_instantiate_factor_builds_instance_from_registry_class --
    def _section_0_test_instantiate_factor_builds_instance_from_registry_class():
    ...
    _section_0_test_instantiate_factor_builds_instance_from_registry_class()

    # -- 原 test_prepare_return_frame_adds_ret_and_forward_returns --
    def _section_1_test_prepare_return_frame_adds_ret_and_forward_returns():
    ...
    _section_1_test_prepare_return_frame_adds_ret_and_forward_returns()
...
def test_equal_weight_and_lgbm_core_suite():
    """等权合成结果不含 null/nan。；等权合成后截面均值接近 0（z-score 均值属性）。；test_lgbm_learns_signal；test_lgbm_deterministic；泄漏探针:扰动 cutoff 后收益,cutoff 前 OOS 预测逐行不变。；test_build_panel_inner_join_and_ret；一个因子全缺 → 丢弃它、用其余因子照常组合(健壮性:不因坏因子崩整个 run)。"""
    # -- 原 test_no_nan --
    def _section_0_test_no_nan():
    ...
    _section_0_test_no_nan()

    # -- 原 test_cross_sectional_mean_near_zero --
    def _section_1_test_cross_sectional_mean_near_zero():
        factor_dfs, _ = _make_factor_ret()
        result = equal_weight(factor_dfs)
    ...
    _section_1_test_cross_sectional_mean_near_zero()

    # -- 原 test_lgbm_learns_signal --
    def _section_2_test_lgbm_learns_signal():
    ...
    _section_2_test_lgbm_learns_signal()

    # -- 原 test_lgbm_deterministic --
    def _section_3_test_lgbm_deterministic():
    ...
    _section_3_test_lgbm_deterministic()

    # -- 原 test_lgbm_no_lookahead --
    def _section_4_test_lgbm_no_lookahead():
    ...
    _section_4_test_lgbm_no_lookahead()

    # -- 原 test_build_panel_inner_join_and_ret --
    def _section_5_test_build_panel_inner_join_and_ret():
    ...
    _section_5_test_build_panel_inner_join_and_ret()

    # -- 原 test_lgbm_drops_all_null_factor_and_continues --
    def _section_6_test_lgbm_drops_all_null_factor_and_continues():
    ...
    _section_6_test_lgbm_drops_all_null_factor_and_continues()
...
def test_purged_cv_suite():
    """test_purge_gap_holds；test_test_segments_contiguous_non_overlapping；test_max_train_before_min_test；test_expanding_train_grows_and_nests；test_rolling_window_fixed_size；test_embargo_widens_gap；test_insufficient_dates_raises"""
    # -- 原 test_purge_gap_holds --
    def _section_0_test_purge_gap_holds():
        dates = _dates(100)
        cv = PurgedWalkForwardCV(train_days=40, test_days=20, purge_days=5)
    ...
    _section_0_test_purge_gap_holds()

    # -- 原 test_test_segments_contiguous_non_overlapping --
    def _section_1_test_test_segments_contiguous_non_overlapping():
        dates = _dates(100)
    ...
    _section_1_test_test_segments_contiguous_non_overlapping()

    # -- 原 test_max_train_before_min_test --
    def _section_2_test_max_train_before_min_test():
        dates = _dates(100)
        for tr, te in PurgedWalkForwardCV(40, 20, 5).split(dates):
            assert max(int(d) for d in tr) < min(int(d) for d in te)

    _section_2_test_max_train_before_min_test()

    # -- 原 test_expanding_train_grows_and_nests --
    def _section_3_test_expanding_train_grows_and_nests():
        dates = _dates(100)
    ...
    _section_3_test_expanding_train_grows_and_nests()

    # -- 原 test_rolling_window_fixed_size --
    def _section_4_test_rolling_window_fixed_size():
        dates = _dates(100)
        folds = PurgedWalkForwardCV(40, 20, 5, expanding=False).split(dates)
        sizes = [len(tr) for tr, _ in folds]
        assert max(sizes) - min(sizes) <= 1  # 滚动窗训练集基本定长

    _section_4_test_rolling_window_fixed_size()

    # -- 原 test_embargo_widens_gap --
    def _section_5_test_embargo_widens_gap():
        dates = _dates(100)
    ...
    _section_5_test_embargo_widens_gap()

    # -- 原 test_insufficient_dates_raises --
    def _section_6_test_insufficient_dates_raises():
        with pytest.raises(ValueError):
            PurgedWalkForwardCV(40, 20, 5).split(_dates(30))

    _section_6_test_insufficient_dates_raises()
...
def test_explain_and_lgbm_robust_suite():
    """test_explain_gain；test_explain_auto_falls_back_to_gain_without_shap；test_explain_unknown_method_raises；库里混入一个物化为空(0 行)的因子(陈旧基本面 ts_skew 退化)→ 丢弃它、用其余因子照常组合。；两因子覆盖的股票**不相交** → inner join 全空(train 与 test 都空)→ 原实现崩。；所有因子都全缺 → 去除退化因子后 0 个有效 → 明确报错(不静默产垃圾)。；线性路径同样容缺:覆盖不相交的两因子外连接 + 缺失补 0(中性)→ 覆盖并集不崩。；因子面板含少量重复 (trade_date, ts_code) 时，链式 outer join 不得笛卡尔积爆炸。；重复键必须告警——静默去重会掩盖上游数据缺陷（本例真实来源仍未查清）。"""
    # -- 原 test_explain_gain --
    def _section_0_test_explain_gain():
        c, x = _fitted()
    ...
    _section_0_test_explain_gain()

    # -- 原 test_explain_auto_falls_back_to_gain_without_shap --
    def _section_1_test_explain_auto_falls_back_to_gain_without_shap(mp):
        c, x = _fitted()
    ...
    with pytest.MonkeyPatch.context() as mp:
        _section_1_test_explain_auto_falls_back_to_gain_without_shap(mp)

    # -- 原 test_explain_unknown_method_raises --
    def _section_2_test_explain_unknown_method_raises():
        c, x = _fitted()
        with pytest.raises(ValueError):
            explain(c, x, method="nonsense")

    _section_2_test_explain_unknown_method_raises()

    # -- 原 test_lgbm_drops_empty_factor_instead_of_crashing --
    def _section_3_test_lgbm_drops_empty_factor_instead_of_crashing():
    ...
    _section_3_test_lgbm_drops_empty_factor_instead_of_crashing()

    # -- 原 test_lgbm_survives_heterogeneous_coverage --
    def _section_4_test_lgbm_survives_heterogeneous_coverage():
    ...
    _section_4_test_lgbm_survives_heterogeneous_coverage()

    # -- 原 test_lgbm_all_null_factors_raise_clear_error --
    def _section_5_test_lgbm_all_null_factors_raise_clear_error():
    ...
    _section_5_test_lgbm_all_null_factors_raise_clear_error()

    # -- 原 test_combine_oos_survives_heterogeneous_coverage --
    def _section_6_test_combine_oos_survives_heterogeneous_coverage():
    ...
    _section_6_test_combine_oos_survives_heterogeneous_coverage()

    # -- 原 test_duplicate_join_keys_do_not_explode_rows --
    def _section_7_test_duplicate_join_keys_do_not_explode_rows():
    ...
    _section_7_test_duplicate_join_keys_do_not_explode_rows()

    # -- 原 test_duplicate_join_keys_emit_warning --
    def _section_8_test_duplicate_join_keys_emit_warning():
        from factorzen.research.combination.methods import _zscore_and_merge
    ...
    _section_8_test_duplicate_join_keys_emit_warning()
