...
def test_run_risk_build_artifacts_suite(tmp_path):
    """test_run_risk_build_writes_artifacts；manifest.json 应含 command/git_dirty/pixi_lock_sha256/schema_version（复用 core.exper；显式传 command 时应原样记录，供复现当时具体怎么跑的。"""
    # -- 原 test_run_risk_build_writes_artifacts --
    def _section_0_test_run_risk_build_writes_artifacts(tmp_path):
    ...
    _section_0_test_run_risk_build_writes_artifacts(_tp0)

    # -- 原 test_run_risk_build_manifest_has_reproducibility_fields --
    def _section_1_test_run_risk_build_manifest_has_reproducibility_fields(tmp_path):
        from factorzen.pipelines.risk_build import run_risk_build
        daily, db, stocks, start, end = _mock()
    ...
    _section_1_test_run_risk_build_manifest_has_reproducibility_fields(_tp1)

    # -- 原 test_run_risk_build_manifest_command_override --
    def _section_2_test_run_risk_build_manifest_command_override(tmp_path):
        from factorzen.pipelines.risk_build import run_risk_build
        daily, db, stocks, start, end = _mock()
    ...
    _section_2_test_run_risk_build_manifest_command_override(_tp2)
...
def test_risk_lookback_and_inputs_suite():
    """lookback 起始日须往前推足够覆盖最长滚动风格因子窗（momentum rolling_sum(252)、；生产回归测试：CLI 若只 fetch [start,end]，窗口首日滚动风格因子全空、；无 lookback 时：W1 一次物化后滚动因子在窗口后段出现，W2 并集固定列集不再因；全窗 raw 物化 → 按 ≤d + universe 标准化 再 build，≡ 单独 build(start,d)。"""
    # -- 原 test_risk_lookback_start_covers_max_rolling_window --
    def _section_0_test_risk_lookback_start_covers_max_rolling_window():
    ...
    _section_0_test_risk_lookback_start_covers_max_rolling_window()

    # -- 原 test_load_risk_inputs_fetches_lookback_so_build_keeps_all_style_factors --
    def _section_1_test_load_risk_inputs_fetches_lookback_so_build_keeps_all_style_factors():
    ...
    _section_1_test_load_risk_inputs_fetches_lookback_so_build_keeps_all_style_factors()

    # -- 原 test_build_no_lookback_still_runs_with_union_styles --
    def _section_2_test_build_no_lookback_still_runs_with_union_styles():
        from factorzen.risk import RiskModel
    ...
    _section_2_test_build_no_lookback_still_runs_with_union_styles()

    # -- 原 test_research_style_reuse_matches_standalone_build --
    def _section_3_test_research_style_reuse_matches_standalone_build():
    ...
    _section_3_test_research_style_reuse_matches_standalone_build()
...
def test_risk_decompose_and_cov_suite():
    """factor_risk² + specific_risk² ≈ total_risk²（方差可加）。；端到端 build（mock 数据，n_days≥280 让 momentum 有值）→ R²∈[0,1]。；回归测试：股票在窗口第3期(非首尾)缺失时，重建残差矩阵不能把缺口前的残差；test_factor_covariance_symmetric_psd；test_specific_risk_positive；test_eigenvector_adjustment_symmetric_same_shape；test_covariance_too_short_returns_identity"""
    # -- 原 test_decompose_risk_variance_conservation --
    def _section_0_test_decompose_risk_variance_conservation():
        result = _toy_result()
    ...
    _section_0_test_decompose_risk_variance_conservation()

    # -- 原 test_build_end_to_end_r_squared_in_range --
    def _section_1_test_build_end_to_end_r_squared_in_range():
    ...
    _section_1_test_build_end_to_end_r_squared_in_range()

    # -- 原 test_build_residual_matrix_mid_window_gap_no_misalignment --
    def _section_2_test_build_residual_matrix_mid_window_gap_no_misalignment():
    ...
    _section_2_test_build_residual_matrix_mid_window_gap_no_misalignment()

    # -- 原 test_factor_covariance_symmetric_psd --
    def _section_3_test_factor_covariance_symmetric_psd():
    ...
    _section_3_test_factor_covariance_symmetric_psd()

    # -- 原 test_specific_risk_positive --
    def _section_4_test_specific_risk_positive():
    ...
    _section_4_test_specific_risk_positive()

    # -- 原 test_eigenvector_adjustment_symmetric_same_shape --
    def _section_5_test_eigenvector_adjustment_symmetric_same_shape():
    ...
    _section_5_test_eigenvector_adjustment_symmetric_same_shape()

    # -- 原 test_covariance_too_short_returns_identity --
    def _section_6_test_covariance_too_short_returns_identity():
        from factorzen.risk.covariance import estimate_factor_covariance
    ...
    _section_6_test_covariance_too_short_returns_identity()
...
def test_industry_dummies_suite():
    """test_industry_dummies_one_hot_per_stock；test_industry_names_sorted_bare；test_industry_dummies_missing_col_raises；缺列填 0、列序对齐固定全集。；合成场景：某行业中途出现——旧逻辑丢日、新逻辑保留且缺列 0。；退化可见性：n_factor_mismatch / n_valid_dates 字段存在且默认可读。"""
    # -- 原 test_industry_dummies_one_hot_per_stock --
    def _section_0_test_industry_dummies_one_hot_per_stock():
    ...
    _section_0_test_industry_dummies_one_hot_per_stock()

    # -- 原 test_industry_names_sorted_bare --
    def _section_1_test_industry_names_sorted_bare():
    ...
    _section_1_test_industry_names_sorted_bare()

    # -- 原 test_industry_dummies_missing_col_raises --
    def _section_2_test_industry_dummies_missing_col_raises():
        import pytest
    ...
    _section_2_test_industry_dummies_missing_col_raises()

    # -- 原 test_reindex_exposure_fills_missing_industry_with_zero --
    def _section_3_test_reindex_exposure_fills_missing_industry_with_zero():
        exp = ExposureMatrix(
            codes=["A", "B"],
    ...
    _section_3_test_reindex_exposure_fills_missing_industry_with_zero()

    # -- 原 test_industry_mid_window_appearance_kept_with_zero_fill --
    def _section_4_test_industry_mid_window_appearance_kept_with_zero_fill():
    ...
    _section_4_test_industry_mid_window_appearance_kept_with_zero_fill()

    # -- 原 test_n_factor_mismatch_visible_on_result --
    def _section_5_test_n_factor_mismatch_visible_on_result():
    ...
    _section_5_test_n_factor_mismatch_visible_on_result()
