...
def test_style_registry_and_size_suite():
    """test_registry_has_eight_named_factors；test_size_factor_shape；test_cs_standardize_rejects_unknown_method"""
    # -- 原 test_registry_has_eight_named_factors --
    def _section_0_test_registry_has_eight_named_factors():
    ...
    _section_0_test_registry_has_eight_named_factors()

    # -- 原 test_size_factor_shape --
    def _section_1_test_size_factor_shape():
    ...
    _section_1_test_size_factor_shape()

    # -- 原 test_cs_standardize_rejects_unknown_method --
    def _section_2_test_cs_standardize_rejects_unknown_method():
        import pytest
    ...
    _section_2_test_cs_standardize_rejects_unknown_method()
...
def test_style_factor_compute_suite(ctx_basic, ctx_daily):
    """test_liquidity_style；test_size_style；test_value_style；test_momentum_style；test_volatility_style；test_beta_style_is_alias_of_beta60d"""
    # -- 原 test_liquidity_style --
    def _section_0_test_liquidity_style(ctx_basic):
    ...
    _section_0_test_liquidity_style(ctx_basic)

    # -- 原 test_size_style --
    def _section_1_test_size_style(ctx_basic):
    ...
    _section_1_test_size_style(ctx_basic)

    # -- 原 test_value_style --
    def _section_2_test_value_style(ctx_basic):
    ...
    _section_2_test_value_style(ctx_basic)

    # -- 原 test_momentum_style --
    def _section_3_test_momentum_style(ctx_daily):
    ...
    _section_3_test_momentum_style(ctx_daily)

    # -- 原 test_volatility_style --
    def _section_4_test_volatility_style(ctx_daily):
    ...
    _section_4_test_volatility_style(ctx_daily)

    # -- 原 test_beta_style_is_alias_of_beta60d --
    def _section_5_test_beta_style_is_alias_of_beta60d():
    ...
    _section_5_test_beta_style_is_alias_of_beta60d()
...
def test_style_panel_and_industry_suite():
    """全窗一次物化再切片 ≡ 逐日 compute_exposures 的风格列（atol 1e-12）。；查证：标准化按 trade_date 分组，两日均值各自≈0。；行业中途出现：并集面板缺列日为 0。"""
    # -- 原 test_materialize_style_panel_matches_per_day_compute --
    def _section_0_test_materialize_style_panel_matches_per_day_compute():
    ...
    _section_0_test_materialize_style_panel_matches_per_day_compute()

    # -- 原 test_cs_standardize_is_by_trade_date_not_pooled --
    def _section_1_test_cs_standardize_is_by_trade_date_not_pooled():
        df = pl.DataFrame({
    ...
    _section_1_test_cs_standardize_is_by_trade_date_not_pooled()

    # -- 原 test_industry_panel_union_fills_missing --
    def _section_2_test_industry_panel_union_fills_missing():
        stocks = pl.DataFrame({
            "ts_code": ["A", "B"],
            "industry": ["银行", "医药"],
        })
    ...
    _section_2_test_industry_panel_union_fills_missing()
...
def test_compute_exposures_suite(caplog):
    """test_compute_exposures_shape_and_factors；行业哑变量每行之和精确为 1.0（每只股票属且仅属一个行业，数学确定量）。；风格因子经截面 Z-score 标准化后，截面均值数学上严格为 0（1e-10 级）。；风格因子 Z-score 后标准差应接近 1（n=8，> 0.1 排除全零 stub）。；PIT 历史行业数据可用时：同一只股票在窗口早期、晚期应按当时实际分类取得；PIT 历史行业数据获取失败（如无权限/网络问题）：compute_exposures 不应崩溃，；降级警告只应触发一次，不能每次 compute_exposures 调用都刷屏。；PIT 历史行业数据只覆盖部分股票代码（如合成代码恰好与真实代码撞号）时："""
    # -- 原 test_compute_exposures_shape_and_factors --
    def _section_0_test_compute_exposures_shape_and_factors():
    ...
    _section_0_test_compute_exposures_shape_and_factors()

    # -- 原 test_compute_exposures_industry_dummies_one_hot --
    def _section_1_test_compute_exposures_industry_dummies_one_hot():
        from factorzen.risk.exposures import compute_exposures
        daily, db, stocks = make_daily(), make_daily_basic__risk_exposures(), make_stocks()
    ...
    _section_1_test_compute_exposures_industry_dummies_one_hot()

    # -- 原 test_compute_exposures_style_factors_zscore_mean --
    def _section_2_test_compute_exposures_style_factors_zscore_mean():
        from factorzen.risk.exposures import compute_exposures
        daily, db, stocks = make_daily(), make_daily_basic__risk_exposures(), make_stocks()
        target = daily["trade_date"].max()
        exp = compute_exposures(daily, db, stocks, target)
    ...
    _section_2_test_compute_exposures_style_factors_zscore_mean()

    # -- 原 test_compute_exposures_style_factors_nontrivial_dispersion --
    def _section_3_test_compute_exposures_style_factors_nontrivial_dispersion():
        from factorzen.risk.exposures import compute_exposures
        daily, db, stocks = make_daily(), make_daily_basic__risk_exposures(), make_stocks()
        target = daily["trade_date"].max()
        exp = compute_exposures(daily, db, stocks, target)
    ...
    _section_3_test_compute_exposures_style_factors_nontrivial_dispersion()

    # -- 原 test_compute_exposures_pit_industry_uses_historical_classification --
    def _section_4_test_compute_exposures_pit_industry_uses_historical_classification(mp):
        from factorzen.risk.exposures import compute_exposures
        ...
        mp.setattr(exposures_module, "fetch_index_member_all", lambda: membership)
    ...
    with pytest.MonkeyPatch.context() as mp:
        _section_4_test_compute_exposures_pit_industry_uses_historical_classification(mp)

    # -- 原 test_compute_exposures_pit_industry_unavailable_falls_back_with_warning --
    def _section_5_test_compute_exposures_pit_industry_unavailable_falls_back_with_warning(mp, caplog):
        caplog.clear()
        from factorzen.risk.exposures import compute_exposures
        ...
        mp.setattr(exposures_module, "fetch_index_member_all", _boom)
        mp.setattr(exposures_module, "_pit_industry_warned", False)
    ...
    with pytest.MonkeyPatch.context() as mp:
        _section_5_test_compute_exposures_pit_industry_unavailable_falls_back_with_warning(mp, caplog)

    # -- 原 test_compute_exposures_pit_industry_warns_only_once --
    def _section_6_test_compute_exposures_pit_industry_warns_only_once(mp, caplog):
        caplog.clear()
        from factorzen.risk.exposures import compute_exposures

        mp.setattr(exposures_module, "fetch_index_member_all", lambda: None)
        mp.setattr(exposures_module, "_pit_industry_warned", False)
    ...
    with pytest.MonkeyPatch.context() as mp:
        _section_6_test_compute_exposures_pit_industry_warns_only_once(mp, caplog)

    # -- 原 test_compute_exposures_pit_industry_partial_coverage_fills_gap_from_stocks --
    def _section_7_test_compute_exposures_pit_industry_partial_coverage_fills_gap_from_stocks(mp, caplog):
        caplog.clear()
        from factorzen.risk.exposures import compute_exposures

        # suite 内 autouse 只在整测入口打一次；前序段可能把 once-flag 置 True，段间重置
        mp.setattr(exposures_module, "_pit_industry_warned", False)
    ...
    with pytest.MonkeyPatch.context() as mp:
        _section_7_test_compute_exposures_pit_industry_partial_coverage_fills_gap_from_stocks(mp, caplog)
