...
def test_job_fetch_daily_calls_loader():
    ...
    with patch("automation.jobs.fetch_daily") as mock_fetch:
        from automation.jobs import job_fetch_daily
...
def test_job_fetch_daily_reraises_on_error():
    ...
    with patch("automation.jobs.fetch_daily", side_effect=RuntimeError("network error")):
        from automation.jobs import job_fetch_daily
...
def test_job_fetch_index_calls_each_benchmark():
    ...
    with patch("automation.jobs.fetch_index_daily") as mock_idx:
        from automation.jobs import job_fetch_index
...
def test_job_fetch_index_reraises_on_error():
    ...
    with patch("automation.jobs.fetch_index_daily", side_effect=ValueError("bad code")):
        from automation.jobs import job_fetch_index
...
def test_job_compute_factors_calls_registry():
    ...
    with patch("daily.factors.registry.get_factor", return_value=mock_factor_cls) as mock_get:
        from automation.jobs import job_compute_factors
...
def test_job_compute_factors_reraises_on_error():
    ...
    with patch("daily.factors.registry.get_factor", side_effect=KeyError("unknown_factor")):
        from automation.jobs import job_compute_factors
...
def test_job_evaluate_calls_subprocess():
    ...
    with patch("automation.jobs.subprocess.run", return_value=mock_result) as mock_run:
        from automation.jobs import job_evaluate

        job_evaluate("20250513", "momentum_20d")
        mock_run.assert_called_once()
        cmd = mock_run.call_args[0][0]
        assert "scripts/run_daily_single.py" in cmd
...
def test_job_evaluate_raises_on_nonzero_returncode():
    ...
    with patch("automation.jobs.subprocess.run", return_value=mock_result):
        from automation.jobs import job_evaluate
...
def test_job_evaluate_reraises_subprocess_exception():
    ...
    with patch("automation.jobs.subprocess.run", side_effect=OSError("pixi not found")):
        from automation.jobs import job_evaluate
...
def test_job_generate_report_calls_subprocess():
    ...
    with patch("automation.jobs.subprocess.run", return_value=mock_result) as mock_run:
        from automation.jobs import job_generate_report

        job_generate_report("20250513", "momentum_20d")
        mock_run.assert_called_once()
        cmd = mock_run.call_args[0][0]
        assert "scripts/generate_report.py" in cmd
        assert "momentum_20d" in cmd
        assert "20250513" in cmd
...
def test_job_generate_report_reraises_subprocess_exception():
    ...
    with patch("automation.jobs.subprocess.run", side_effect=FileNotFoundError("no pixi")):
        from automation.jobs import job_generate_report
