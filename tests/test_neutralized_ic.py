...
def test_neutralized_ic_returns_ic_stats():
    ...
    from factorzen.daily.evaluation.advanced import compute_neutralized_ic
    from factorzen.daily.evaluation.ic_analysis import IcStats
...
def test_neutralized_ic_handles_missing_industry_labels():
    """缺失行业标签不应导致行业哑变量排序失败。"""
    from factorzen.daily.evaluation.advanced import compute_neutralized_ic
    from factorzen.daily.evaluation.ic_analysis import IcStats
            ...
            rows.append(
                {
                    ...
                    "factor_clean": float(rng.standard_normal()),
                    "ret_1d": float(rng.standard_normal() * 0.01),
                    "industry": None if s % 10 == 0 else f"ind_{s % 4}",
                    "total_mv": None if s % 13 == 0 else float(rng.uniform(1e8, 1e10)),
                }
            )

    result = compute_neutralized_ic(pl.DataFrame(rows))
